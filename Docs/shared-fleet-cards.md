...
**Chief note:** Shared living-host **amend DONE ~22:08** (CLEAR). **CLEAR→OPEN ~07:06** re-queued an OPEN amend; **CLEAR recovered ~07:25** first → **cancel** that OPEN-since-**~07:06** queue. Shared **CLEAR since ~07:25** then **queued**. Living **CLEAR→OPEN again ~08:41** **before** Shellable — that CLEAR amend **never wrote**. **Missed CLEAR window.** **Cancel / supersede** the Shared **CLEAR ~07:25** queue **now** — do **not** leave stale “write CLEAR since ~07:25” across the flap. Stamp **`missed_clear_window={start:~07:25,end:~08:41,reason:empty}`**. Do **not** backfill that missed CLEAR as current living-host once OPEN. Shared amend stays **HOLD** until the **next** CLEAR recover + ≥30m SAME-CLASS + Docs **CONNECTED+Shellable**. Do **not** write OPEN-since-**~08:41** mid-flap. **`shared_fleet_card_stale=true`**. Shared still **never** overrides Docs/#70 SSOT. Essay: [missed-clear-window-shared-supersede.md](missed-clear-window-shared-supersede.md) · [soft530-clear-vs-local-exec.md](soft530-clear-vs-local-exec.md) · [freeze-build-id-public-probe.md](freeze-build-id-public-probe.md).
...
| **Don’t treat Shared as SSOT** | Amend **DONE ~22:08** is **stale** vs living **OPEN ~08:41**. OPEN-since-**~07:06** queue **cancelled**. **CLEAR since ~07:25** queue **superseded** (missed window). Do **not** write OPEN-since-**~08:41** mid-flap. Do **not** backfill the missed CLEAR. Overnight stamp **`shared_fleet_card_stale=true`**. Shared still **non-SSOT**. Docs/#70 is SSOT. |
...
| **Don’t keep “in progress” / don’t land cancelled or superseded queues** | The rewrite landed ~15:42. The living-host amend landed **~22:08**. The **~07:06** OPEN queue is **cancelled**. The **~07:25** CLEAR queue is **superseded**. Do **not** write OPEN-since-**~08:41**. Do **not** backfill **CLEAR since ~07:25**. Next amend = next CLEAR recover + ≥30m + Shellable — **not** mid-flap. |
