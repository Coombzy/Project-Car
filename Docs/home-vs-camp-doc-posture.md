...
| **HOME-awake short-blip recover** (living ~07:25 / EMPTY) | ListMachines **EMPTY** | Living **CLEAR** (`/health` **200**; `app.`/`ops.` **307**; **`soft530_clear_since` ~07:25**; **`soft530_open_blip` ~07:06–~07:25**) | Independently **OPEN 530 / CF 1033** connector-down (was 502) — **EXPECTED** | **Half-state.** **CONNECTED ≠ Shellable ≠ tunnel healthy.** **Do not** enter [post-dual-clear-go.md](post-dual-clear-go.md). **Must not reopen** camp GO. Re-offer only after **CONNECTED+Shellable** + clear-smoke + ≥30m SAME-CLASS dwell ([soft-530-blip-recover.md](soft-530-blip-recover.md)). Ben asked once — **no re-nag**. Lookout api `ok=true` / vault `ok=false` **expected**. |
