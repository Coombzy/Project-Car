...
| **Shared** | **HOLD.** Do **not** write OPEN-since-**~08:41** mid-flap. OPEN-since-**~07:06** queue stays **cancelled**. **CLEAR since ~07:25** queue **cancelled** — home half-ops (**not** missed camp GO). **`missed_clear_window` omit.** Do **not** backfill that CLEAR. Amend Shared **only** after next CLEAR recover + ≥30m SAME-CLASS + Docs **CONNECTED+Shellable**. **`shared_fleet_card_stale=true`**. |
...
- Amend Shared living-host **mid-flap**, land the cancelled **OPEN since ~07:06** queue, land the cancelled **CLEAR since ~07:25** queue, backfill **CLEAR since ~07:25**, stamp `missed_clear_window` for home half-ops, or write **OPEN since ~08:41** before the next CLEAR recover + ≥30m + Shellable
