...
**Now:** [projectcar.ca](https://projectcar.ca) brochure — Cloudflare Worker **`projectcar-brochure`** Direct Upload (`brochure-worker-deploy.md`; Home, About, The Shop, Membership, Roadmap, Contact) + public waitlist (`POST https://api.projectcar.ca/waitlist`). Apex sidecar **deferred** (Ben). Classic Pages git is **plan only**. Shop OS (Owner + Member booking, calendar #18, fill #20, placeholders #21, inventory #26, Chat v1 #27, Dashboard #28) is **LIVE** on **`ops.projectcar.ca`** → Doc `:3000`. `app.projectcar.ca` is a **temporary alias** for the same origin. Member demo still `/member` on that UI. The shop is not open.
...
- **Agents** — two planes in [`ai-agents-constitution.md`](ai-agents-constitution.md): **Grok Bot fleet** (coding + ops) and **Hermes / hardware** (Doc, Porsche, McKing, Code Mater). Do **not** treat Porsche as the coding supervisor. Discord + STATUS + Lead standing GO. **No n8n.**
...
|-----|---------------|-------------|
| Nextcloud / MC | Yes — already daily-usable | Cockpit UI (**parked** until **Ben GO**; Owner + Member booking already live on Doc) |
| projectcar.ca | Yes | Waitlist API (live); origin is Worker **`projectcar-brochure`** Direct Upload |
| Shop OS | Yes (Owner tool on `main`) | **LIVE** on **`ops.projectcar.ca`** → Doc `:3000`; `app.` is a temporary alias; later member login |

---

**Updated 2026-09-07** to match shipped reality: waitlist + Owner shop OS are on `main`; Apex deferred; live brochure is Worker **`projectcar-brochure`** Direct Upload (`brochure-worker-deploy.md`); Classic Pages git is **plan only** (`brochure-pages-cutover.md`) — not started, not GO’d; management = **`ops.projectcar.ca` LIVE**; `app.` = temporary alias; MC cockpit **parked** until **Ben GO** (booking-live hold already satisfied). Agents: constitution two-plane (Grok coding+ops + Hermes/hardware) — Porsche is not the coding supervisor. Locks unchanged: no Stripe / no “shop is open”; members ≠ Nextcloud; no n8n.
