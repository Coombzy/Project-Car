...
**Updated:** 2026-09-15  
**Related:** `STATUS.md` (living dual-OPEN + this pointer), [camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md) (**camp** vault path — Doc VW + Zone retarget; **this hop is home/lab only**; do **not** treat McKing docker as camp vault recovery), [overnight-baseline-stamp.md](overnight-baseline-stamp.md) (if this hop completes between last weekday `*/20` and morning first `*/20`, morning **must not** re-surface the ListMachines DIFF to Ben — **refresh** the living stamp; optional Lead-only ack; same-class quiet `611c267`), [soft-530-extended-open.md](soft-530-extended-open.md) (quiet-ops lock while OPEN **~24h+** + vault OPEN — this file is the **first hop** when McKing reappears; quiet-ops **HOLD** after hop if HTTP class unchanged), [soft-530-weekday-overnight.md](soft-530-weekday-overnight.md) (Mon–Thu overnight gap — mid-overnight McKing reappear still **this** hop; **never** `Mac.lan`; **never auto-fire**), [vault-clear-smoke.md](vault-clear-smoke.md) (**first** recovery smoke after identity — `/alive` **200** + `/api/config` VW **2026.6.0** + `docker.service` **enabled** + **active** + classify **502→200**; HTTP-only ≠ CLEAR), [vault-stay-up.md](vault-stay-up.md) (Chief process wake + vault **post-CLEAR stay-up evidence** `@d88cacb`), [dual-host-outage.md](dual-host-outage.md) (wake order — **home/lab** Doc first, then McKing; this file is the **McKing identity hop**), [doc-reappear-first-hop.md](doc-reappear-first-hop.md) (Doc twin — **independent** at home/lab; Lead `Docs-MacBook-Pro` lane), [post-dual-clear-go.md](post-dual-clear-go.md) (**CLEAR path** — ordered Ben GO menu after **both** CLEARs + forensics, **never auto-fire**; **does not** assume this hop first), [soft-530-clear-smoke.md](soft-530-clear-smoke.md) (Soft-530 first card — **not** this hop), [deployment-guide.md](deployment-guide.md) (Monday paper map), [home-lab-specification.md](home-lab-specification.md) (McKing = `lightning`; Porsche = travel **client**, never McKing; **home/lab vs camp** dual-tunnel)

When **`lightning` / McKing first reappears** on ListMachines after **extended vault OPEN / quiet-ops** **at home/lab**, run **this** hop **before** Bitwarden / desk / Soft-530 CLEAR talk / unfreeze / **#82**. Quiet-ops named the lock ([soft-530-extended-open.md](soft-530-extended-open.md)). This file names the **home/lab first hop** — identity, then smoke, then (only if CLEAR) forensics, then Soft-530 independence.

**Camp vault recovery is not this hop.** After Ben GOs [camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md), public vault is Doc VW + Zone retarget. McKing **may sleep**. A `lightning` name on ListMachines is **lab** identity only — do **not** treat a McKing docker ask as the camp vault path. [post-dual-clear-go.md](post-dual-clear-go.md) does **not** wait on this hop.

This fold executes **none** of the curls, Shells, hops, or GOs. Do **not** invent vault CLEAR, Soft-530 CLEAR, **#82**, unfreeze, Zone Direct Upload, Garage **#79.1**, a companion re-ask, a Ben docker / Doc-wake re-ask, or Bitwarden from this paper.
...
| **2** | **Run [vault-clear-smoke.md](vault-clear-smoke.md)** | After Chief vault wake on the **asserted** host ([vault-stay-up.md](vault-stay-up.md) — VW + McKing `cloudflared` only): `GET https://vault.projectcar.ca/alive` **200**; `GET https://vault.projectcar.ca/api/config` **200** + Vaultwarden **2026.6.0** class (**reject** Doc sibling **2025.12.0**); on `lightning` `systemctl is-enabled docker` == **enabled** **and** `is-active` == **active** (optional: `mcking` in `docker` group); classify this OPEN **502→200** (origin-down), **not** CF **1033→200**. `/alive` **200** + docker **disabled/inactive** = **NOT CLEAR / recurrence risk** — reboot re-OPENs vault the same way as ~19:45 Sep 13 (disabled + SIGKILL stop). HTTP-only is **not** CLEAR. |
...
|-------------------|---------|
| **Bitwarden gate is vault CLEAR + Ben GO** | After [vault-clear-smoke.md](vault-clear-smoke.md) (`/alive` **200** + VW **2026.6.0** + `docker.service` **enabled** + **active**), import/rotate is **Ben GO** — **not** blocked on Soft-530 CLEAR. Soft-530 **OPEN** can coexist with vault CLEAR + Bitwarden work. **Never auto Bitwarden.** Not on [post-dual-clear-go.md](post-dual-clear-go.md). |
...
| **Reappear ≠ CLEAR** | Host online is not `/alive` **200** + `/api/config` **2026.6.0** + `docker.service` **enabled** + **active** + **502→200**. |
| **`/alive` 200 ≠ right VW** | Need `/api/config` **200** + McKing **2026.6.0** class (not Doc **2025.12.0**). |
| **`/alive` 200 ≠ docker permanence** | Need `systemctl is-enabled docker` == **enabled** **and** `is-active` == **active** on `lightning`. `/alive` **200** + docker **disabled/inactive** = **NOT CLEAR / recurrence risk** (reboot re-OPENs like ~19:45 Sep 13). |
...
| **Auto Bitwarden** | Gate is **vault CLEAR** (Ben GO after `/alive` + VW **2026.6.0** + docker permanence) — **not** Soft-530 CLEAR. This hop does **not** auto-start a client import, vault-item rotate, bulk re-key, or official Bitwarden → Vaultwarden move. |
...
| **This hop = camp vault path** | Camp vault recovery = Doc VW + Zone retarget ([camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md)). McKing docker is **home/lab**. |
| **Sequencer waits on this hop** | [post-dual-clear-go.md](post-dual-clear-go.md) does **not** assume McKing vault wake first. |
...
- Treat this hop / McKing docker as the **camp** vault path ([camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md))
...
- Treat `/alive` **200** as hop-**2** CLEAR without `docker.service` **enabled** + **active** ([vault-clear-smoke.md](vault-clear-smoke.md))
- Re-ask Ben docker / Doc-wake from this paper (already asked — duration / `lightning` back ≠ a new page)
- Auto-fire Bitwarden import/rotate from vault CLEAR — Bitwarden is **Ben GO** after vault CLEAR + `/alive` + VW **2026.6.0** + docker permanence; **not** blocked on Soft-530 CLEAR
