...
## Living now (~07:06 America/Edmonton)

Soft-530 **OPEN 530 / CF 1033** **`soft530_open_since` ~07:06** (Lookout api **200→530** CF **1033**; body error code **1033**; Lead `api.`/`app.`/`ops.` all **530/1033**). Prior CLEAR dwell **~22:00–07:06** (~9h) **ENDED**. Vault still **OPEN 530 / CF 1033**. Docs `95a229f5` **CONNECTED+Shell-unreachable** — **flap-while-connected** (not full **ABSENT**). **CONNECTED ≠ Shellable ≠ tunnel healthy.** Option A Redirect+UX **PASS** (`styles.css?v=36`, `/shop`→`the-shop`). Home **STALE** until **#82**. Freeze **`last_known` `4cf8924` / `5swmVz`**. Lookout api+vault **armed** (`ok=false` / `ok=false` **expected** dual OPEN). Companions **HOLD**. Ben asked once — **no re-nag** / **≠ auto lid-restore nag**. Quiet-ops **HOLD**. **#82** / **#79.1** / unfreeze / camp **not** auto-GO. Camp vault-on-Doc GO window **paused** while OPEN. **`shared_fleet_card_stale=true`**. Dual OPEN both 1033 ≠ dual CLEAR.
