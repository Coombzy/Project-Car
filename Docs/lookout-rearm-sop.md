...
| **Home-off Soft-530 `ok=false`** | Docs **ABSENT** at home → Soft-530 `/health` non-200 / CF **1033** is the **expected baseline** ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)). Stay armed. Do **not** re-arm Soft-530 companions. | Stickiness panic. A new chronic incident. Camp cutover failure. Companion GO. |
...
**Overnight / weekend Option A Dynamic redirect is UNCOVERED — accept that risk.** Weekday Chief `*/20` is **weekday-hours coverage only**. After last weekday `*/20` (~17:40 MT) until the next weekday first fire (~06:00) — [soft-530-weekday-overnight.md](soft-530-weekday-overnight.md) — and Sat/Sun while plan-improve is **off** (`84107e7` weekend stay-up rows), there is **no** Option A redirect smoke. **Mid-weekday addendum:** when Soft-530 is **OPEN** **and** this watch is **GONE**, a weekday `*/20` gap **>~40m** is also **mid-weekday UNCOVERED** — living **~08:49→~11:29** 2026-09-16 ([plan-improve-weekday-coverage-hole.md](plan-improve-weekday-coverage-hole.md)). Last weekday `*/20` writes [overnight-baseline-stamp.md](overnight-baseline-stamp.md) (Lookout watch state **GONE/armed/chronic HOLD** is a stamp field) — **refresh only on field DIFF**; else prior stamp **stands**. First weekday `*/20` after ~06:00 **diffs** that stamp — **same-class = stay quiet**; surface **Lead/Ben only on class DIFF**; do **not** rediscover; do **not** Ben-nag from an unchanged `GONE`. Do **not** invent overnight/weekend/mid-weekday plan-improve, Zone, or Garage coverage to fill that gap. **Next Option A smoke** = the recovering plan-improve fire (Redirect **10/10** + Soft-530 UX root) **or** Ben-approved Lookout recreate. This paper **HOLDs** a second Ben recreate ask (row 3 already one-ask). **Redirect-only PASS ≠ Soft-530 UX PASS.** Duration ≠ recreate GO ≠ **#82** ≠ unfreeze ≠ companion re-ask ≠ honesty-off.
...
| Brochure Option A continuous watch | **GONE.** Paper continues. Weekday interim = Chief `*/20` redirect **+ Soft-530 UX**. Overnight/weekend Dynamic redirect is **UNCOVERED — accept that risk.** Mid-weekday `*/20` gap **>~40m** while Soft-530 **OPEN** is also **UNCOVERED** ([plan-improve-weekday-coverage-hole.md](plan-improve-weekday-coverage-hole.md)). **Next Option A smoke** = the recovering plan-improve fire **or** Ben-approved Lookout recreate. **HOLD** a second Ben recreate ask. **Redirect-only PASS ≠ Soft-530 UX PASS.** |
...
| **Weekday interim ≠ armed; overnight/weekend/mid-weekday UNCOVERED** | Weekday Chief `*/20` is smoke coverage, not `enabled:true`. Overnight + weekend Option A Dynamic redirect is **uncovered — accept that risk**. A weekday `*/20` gap **>~40m** while Soft-530 **OPEN** + watch **GONE** is **mid-weekday UNCOVERED**. Do **not** invent overnight/mid-weekday plan-improve, Zone, or Garage coverage. |
...
| **Not home-off `ok=false` stickiness panic** | Docs **ABSENT** at home → Soft-530 Lookout `ok=false` is **expected baseline** ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)). Do **not** re-arm companions. Do **not** treat home 1033 as camp cutover failure. |
...
| **Not invent overnight/weekend/mid-weekday redirect smoke** | Do **not** invent overnight/mid-weekday plan-improve, Zone, or Garage Option A coverage. **Next Option A smoke** = the recovering plan-improve fire **or** Ben-approved Lookout recreate. |
...
- Treat Lookout Soft-530 `ok=false` while Docs **ABSENT** at home as stickiness panic or camp cutover failure ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md))
