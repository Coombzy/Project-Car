...
| **`Mac.lan` drop (Ben laptop) + HTTP unchanged** | Living vs ~12:24: `Mac.lan` dropped; only **`lightning` CONNECTED**; Docs still **ABSENT**. Soft-530 still **1033**; vault still **502**. **Refresh** the living stamp. **No Ben surface.** | Treat laptop leave as Doc-wake or McKing wake. Page Ben. Fold it into Soft-530 / vault HTTP class. |
...
| **ListMachines** | **`lightning` CONNECTED only** — `Mac.lan` dropped vs ~12:24 America/Edmonton 2026-09-15; `docs=ABSENT`. `Mac.lan` ≠ `Docs-MacBook-Pro` ≠ `lightning`. HTTP class unchanged (Soft-530 **1033** / vault **502**) — **no Ben surface**. McKing first-hop already ran ~19:10 MT Mon 2026-09-14 — still blocked on Ben sudo. **No morning re-ask.** |
