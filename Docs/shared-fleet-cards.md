...
**Evidence 2026-09-15:** Shared rewrite **~15:42** stamped Soft-530 **CLEAR**. Live **OPEN** since **~16:36** with ListMachines **EMPTY** (Lead-verified **~16:51** after flap-while-connected). CONNECTED+Shellable **failed** after the CLEAR→OPEN flip. Overnight stamp tonight Soft-530=**OPEN** + **`soft530_open_since` ~16:36** + **`shared_fleet_card_stale=true`** ([overnight-baseline-stamp.md](overnight-baseline-stamp.md)). Shared living-host line still carries ~15:42 **CLEAR** — **queued** for next Doc Shellable. This fold records the queue; it does **not** execute the Shared file.
...
| **Overnight stamp** | Tonight Soft-530=**OPEN** + **`soft530_open_since` ~16:36** + **`shared_fleet_card_stale=true`** ([overnight-baseline-stamp.md](overnight-baseline-stamp.md)). Do **not** rewrite that stamp from Shared **CLEAR**. Morning first-hop amends Shared **before** treating Shared as living. |
...
| **Don’t leave Shared CLEAR overnight** | ~15:42 rewrite stamped **CLEAR**; live **OPEN** since **~16:36** / ListMachines **EMPTY**. Overnight stamp **`shared_fleet_card_stale=true`**. Overnight/morning readers use Docs SSOT, not Shared CLEAR. |
...
- Rewrite tonight’s overnight stamp away from Soft-530=**OPEN** + **`soft530_open_since` ~16:36** + **`shared_fleet_card_stale=true`**
