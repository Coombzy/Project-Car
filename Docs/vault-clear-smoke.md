...
**Updated:** 2026-09-15  
**Related:** `STATUS.md` (living Soft-530 **OPEN** + vault **OPEN** dual-OPEN both 1033 + **vault OPEN subclass** + this pointer), [camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md) (**camp** smoke = `/alive` **200** + `/api/config` VW class after Zone retarget **Z5** — **not** McKing docker permanence; **Doc local VW/NC healthy ≠ this card** — SSOT stays the **public** hostname; never substitute Doc loopback), [mcking-reappear-first-hop.md](mcking-reappear-first-hop.md) (**home/lab first hop** when `lightning` / McKing reappears — machineId `9067d14b-46e5-4ef0-82d5-fce0febdc8f7`; **never** `Mac.lan`; **then** this smoke — **502** CLEAR is HTTP **+** `docker.service` permanence), [vault-stay-up.md](vault-stay-up.md) (McKing wake + **502 vs 530/1033 triage** + **post-CLEAR stay-up evidence** `@d88cacb` — this file is the **next** vault CLEAR card **at home/lab**; living **1033** first check is lightning Healthy / cloudflared), [overnight-baseline-stamp.md](overnight-baseline-stamp.md) (stamp vault **subclass**; flip = Lead/Lookout field DIFF ≠ Ben re-nag), [soft-530-clear-smoke.md](soft-530-clear-smoke.md) (Soft-530 twin — **already** `502ab2e`; this file is **not** that smoke), [soft-530-extended-open.md](soft-530-extended-open.md) (quiet-ops while Soft-530 OPEN **~24h+** + vault OPEN), [post-dual-clear-go.md](post-dual-clear-go.md) (**CLEAR path** — ordered Ben GO menu, **never auto-fire**; Bitwarden is **not** on that menu; **does not** assume McKing vault wake first), [dual-host-outage.md](dual-host-outage.md) (**home/lab** wake order — Doc first, then McKing; subclass recovery fork), [doc-reappear-first-hop.md](doc-reappear-first-hop.md) (Doc twin — **independent** at home/lab)

After vault **CLEAR** following **extended OPEN**, run this **first recovery smoke BEFORE** any Bitwarden import/rotate or desk talk. Quiet-ops named the lock ([soft-530-extended-open.md](soft-530-extended-open.md)). **Stamp vault OPEN subclass first** ([overnight-baseline-stamp.md](overnight-baseline-stamp.md)):

| Living subclass | Public class | First hop (before this smoke) | This card’s permanence |
|-----------------|--------------|-------------------------------|------------------------|
| **502** | origin-reachable-tunnel | `docker.service` **enabled** + **active** on `lightning` | HTTP **+** docker permanence (`@367172d`) — **502→200** |
| **530 / CF 1033** (living ~16:28) | connector-down | **lightning-mcking Healthy / cloudflared first** ([vault-stay-up.md](vault-stay-up.md)) | Only after connector is up. Do **not** run docker permanence as the 1033 first hop. Classify **1033→200** when that path recovers. |

Vault **OPEN** began **502** since 2026-09-13 **~19:45** America/Edmonton (during Soft-530 **~24h+** quiet-ops). Living subclass (Lead-verified **~16:28** America/Edmonton) is **530 / CF 1033**. Soft-530 **CLEAR** + vault **1033** is still **half-state** ≠ dual CLEAR ≠ auto camp / **#82** / unfreeze / Bitwarden. Subclass flip = Lead/Lookout field DIFF — **not** a Ben re-nag if already asked once. **Home/lab:** this file names the first public proof that **McKing Vaultwarden** is back — not only that `/alive` answered, **and** (on the **502** path) that `docker.service` will stay up across reboot, and not a Doc VW sibling. **Camp** (after Ben GO + Zone retarget): smoke is `/alive` **200** + `/api/config` VW class on **Doc** — [camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md). Do **not** require McKing docker permanence as camp CLEAR. **SSOT stays `https://vault.projectcar.ca`.** Doc loopback VW (`:8222` `/alive`) + NC (`:8080`) after cold-boot is camp **Z1 preflight only** — **never** substitute that loopback for this card; local healthy **never scores** CLEAR. Bitwarden is still **vault CLEAR** + **Ben GO**.
...
|------|------------------------|
| **Prior OPEN was extended** | Vault **OPEN** since 2026-09-13 **~19:45** America/Edmonton on `/alive` + `/api/config`. **Subclass living:** **530 / CF 1033** connector-down (Lead-verified **~16:28**; **was 502**). Quiet-ops: [soft-530-extended-open.md](soft-530-extended-open.md). |
...
Living this fold: Soft-530 **OPEN 530 / CF 1033** (`api.` / `app.` / `ops.`; Lookout + Lead **~16:36**; **was CLEAR**). Vault independently **OPEN** subclass **530 / CF 1033** connector-down (Lead-verified **~16:28** America/Edmonton; **was 502** since ~19:45 MT Sep 13 — **not** CLEAR). Soft-530 **OPEN 1033** + vault **1033** is dual OPEN (half-state **OVER**). ListMachines **EMPTY** / Docs **ABSENT** (Lead-verified **~16:51** after flap-while-connected); lightning **ABSENT** (McKing sleep intentional); `Mac.lan` may CONNECTED. Lookout: api `ok=false` **expected**; vault `ok=false` **expected** (dual OPEN both 1033). Option A Redirect **PASS**. Soft-530 UX **LIVE**. Home **STALE** until **#82**. Freeze `` `changeme` `` / **`5swmVz`**. **#82** still Ben GO. Soft-530 **CLEAR ≠** this card ≠ dual CLEAR ≠ camp GO ≠ Bitwarden. **Doc local VW/NC healthy ≠ this card** — Doc loopback **200** + public `vault.projectcar.ca` **530 / 1033** = still vault **OPEN** (not partial CLEAR / not dual CLEAR / not camp progress %). SSOT stays the public hostname; **never** substitute Doc loopback. This paper does **not** invent vault CLEAR or re-ask Ben docker. First 1033 check is lightning Healthy / cloudflared — Chief checking separately.

**CLEAR path is still** [post-dual-clear-go.md](post-dual-clear-go.md) — **do not enter** until this vault smoke also passes. Living Soft-530 **OPEN** + vault **OPEN** is **dual OPEN both 1033** (half-state **OVER**), not dual CLEAR. After a **live** Soft-530 CLEAR **and** a **live** vault CLEAR **and** both forensic papers (`55e10d0` / `d88cacb`), Ben’s GO menu is **ordered** and **never auto-fire**. This smoke does **not** skip KeepAlive, does **not** auto-GO **#82**, does **not** unfreeze, and does **not** auto-fire Bitwarden.
...
| # | Assert | Expect |
|---|--------|--------|
| **1** | `GET https://vault.projectcar.ca/alive` | **200**. **Public hostname only.** Never substitute Doc loopback `:8222`. Doc `/alive` **200** is [camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md) **Z1 preflight only** — **not** this row. |
...
| **4** | Classify the recovery path | Follow **living subclass**. **502** path = origin-down **502→200** + docker permanence. **530 / CF 1033** path (living) = connector-down **1033→200** after lightning Healthy / cloudflared first — **not** docker as first hop. Soft-530 tunnel class ([soft-530-clear-smoke.md](soft-530-clear-smoke.md) — `502ab2e`) stays **separate**. Do **not** collapse the subclasses. |
...
# SSOT = public hostname — never substitute Doc loopback :8222
...
**Recovery class (row 4):** living this fold is vault **530 / CF 1033** connector-down (still OPEN; **was 502**). Soft-530 is independently **CLEAR** (`/health` **200**) — half-state, **not** this card. **1033 first check** = lightning Healthy / cloudflared ([vault-stay-up.md](vault-stay-up.md)) — Chief checking separately. Do **not** run `docker.service` enabled+active as the 1033 first hop. The **502** docker-permanence path (`@367172d`) remains for origin-reachable-tunnel OPEN. Stamp **1033→200** or **502→200** from the subclass that actually recovered. `/alive` **200** with docker **disabled/inactive** is still **NOT CLEAR / recurrence risk** on the 502 path — same class as ~19:45 Sep 13 (disabled + SIGKILL stop).
...
| **Invent vault CLEAR / invent dual CLEAR** | Soft-530 is **CLEAR**. Vault still **OPEN** (**530 / CF 1033**). ListMachines **EMPTY** / Docs **ABSENT** (Lead-verified **~16:51** after flap-while-connected); lightning **ABSENT**. Soft-530 **OPEN 1033** + vault **1033** is dual OPEN (half-state **OVER**) ≠ this card. |
| **Collapse 502 and 530/1033** | Recovery forks. 1033 → lightning Healthy / cloudflared first. 502 → docker.service enabled+active. Subclass flip ≠ Ben re-nag. |
| **Substitute Doc loopback for this smoke** | SSOT stays `https://vault.projectcar.ca`. Doc `:8222` `/alive` + NC `:8080` after cold-boot is [camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md) **Z1 preflight only** — local healthy **never scores** CLEAR. Living Doc VW up + public **502** = still vault **OPEN** (not partial CLEAR / not dual CLEAR / not camp progress %). |
| **Auto Zone retarget / Bitwarden / McKing docker as camp path** | Paper only. **Ben GO** for retarget. Bitwarden still vault CLEAR + Ben GO. Camp path is Doc VW + Zone retarget — **not** McKing docker. Never `Mac.lan` as Doc/vault. Soft-530 **CLEAR** alone still **≠** camp GO. |
| **Run docker permanence as the 1033 first hop** | Living subclass is connector-down. Soft-530 tunnel class stays separate. |
...
- Classify living **530 / CF 1033** as origin-down **502→200**, or run docker permanence as the 1033 first hop
...
- Substitute Doc loopback / `:8222` for `https://vault.projectcar.ca` — this card’s SSOT is the **public** hostname
- Score Doc VW/NC healthy as this smoke, vault CLEAR, partial CLEAR, dual CLEAR, or camp progress %
- Auto Zone retarget from a green Doc loopback
