...
**Updated:** 2026-09-14  
**Related:** `STATUS.md` (this pointer + Lookout rows + **#78 LIVE-SUPERSEDED** honesty), [brochure-redirect-watch.md](brochure-redirect-watch.md) (Option A watch **spec** — living watch is **GONE**), [soft-530-extended-open.md](soft-530-extended-open.md) (quiet-ops — Lookout api+vault **continue**; brochure continuous watch **does not**), `api-stay-up.md` (**#78 lookout-resume** is **LIVE-SUPERSEDED** — api-health already `enabled:true`), `vault-stay-up.md` (vault `/alive` **LIVE/armed** — **not** this recreate), `shop-web-stay-up.md` (Soft-530 companions **HOLD / not armed**)
...
Quiet-ops ([soft-530-extended-open.md](soft-530-extended-open.md)) still holds: Soft-530 **OPEN** CF **1033** since 2026-09-13 ~11:57 America/Edmonton (**~24h+**); vault independently **OPEN** **502** since ~19:45 MT Sep 13; ListMachines **Mac.lan only**. Quiet-ops says Lookout **continues** — that means **api + vault stay armed**, and the brochure **paper** continues. The continuous brochure watch is **GONE**. Interim coverage is Chief `*/20` redirect smoke (row 4). Duration ≠ recreate GO ≠ **#82** ≠ unfreeze ≠ companion re-ask.
...
| **4** | Interim Option A coverage = Chief plan-improve **`*/20` redirect smoke** until recreate is enabled | Continuous watch is **GONE**. Until Ben enables recreate, Chief keeps the existing `*/20` Option A redirect smoke ([brochure-redirect-watch.md](brochure-redirect-watch.md) asserts). That is **coverage**, not an armed flip watch. Homepage-only **200** is still insufficient. |
...
| Brochure Option A continuous watch | **GONE.** Paper continues. Interim = Chief `*/20` until recreate is enabled. |
...
| **Not companion re-ask** | Companions stay **HOLD / not armed**. |
...
| Anti-goal | Why |
|-----------|-----|
| **Not #82** | Worker-live / Direct Upload is still Ben GO after dual CLEAR. Recreate ≠ upload. |
...
| **Not companion re-ask** | Soft-530 companions stay **HOLD**. Do **not** re-ask. |
