...
- **Brochure P2-1:** **LIVE** on Worker `projectcar-brochure` (2026-09-07 live verify). Garage PR #55 → `453e44d`. Absolute `og:url` / `og:image` + `twitter:card`. Zone smoke PASS. Only P2 item live-verified on Worker.
- **Brochure P2-2:** apex `<link rel="canonical">` tags landed in git this PR (paths match P2-1 `og:url`; host `https://projectcar.ca`, not www). Git/local verify only — do **not** invent Worker-live. P2-3 / P2-4 stay **open** (CSS `?v=` still lags).
...
Two gates — do **not** collapse them into “v1 includes everything forever.” Calendar (#18), fill (#20), placeholders (#21), cookies (#22), Docs (#23 / #25 / **#30–#55**, website **#52–#55**), schedule harden (#24), inventory (#26), Chat (#27), Dashboard (#28), shop-web host allowlist (**#36**, `f952cd3`), brochure P1-1 / P1-3 (**#52**, `5361212`), and brochure P2-1 (**#55**, `453e44d`) are **on `main`** — they are not Next. P2-2 canonicals land in this PR (git/local verify; not Worker-live). P2-3 / P2-4 stay open. **#36 is on `main` only** until Doc pull + rebuild. **`ops.projectcar.ca` is LIVE** at the edge. `app.` remains the temporary alias — do not cut it here.
