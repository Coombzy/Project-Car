...
After **extended OPEN**, the **first** public card is [vault-clear-smoke.md](vault-clear-smoke.md) — **before** Bitwarden / desk. Follow **living subclass**: **530 / CF 1033** (living) → lightning Healthy / cloudflared first, then smoke + classify **1033→200**; **502** → `/alive` **200** + `/api/config` McKing **2026.6.0** class (not Doc **2025.12.0**) + `docker.service` permanence + classify **502→200**. Do **not** collapse the forks. Bitwarden stays **Ben GO**, **never auto-fire**. Essay: [vault-stay-up.md](vault-stay-up.md). Soft-530 `/health` **200** is **not** vault CLEAR. After that CLEAR, stamp **vault post-CLEAR stay-up evidence** (cloudflared.service ActiveState/Result/ExecMainStatus · VW healthy + `DOMAIN=https://vault.projectcar.ca` · Tailscale · **1033→200** vs **502→200**) on [vault-stay-up.md](vault-stay-up.md) before walking away. Forensic paper already ack’d (`d88cacb`).

Living this fold — **dual OPEN both 1033** honesty (do **not** invent dual CLEAR): Soft-530 **OPEN 530 / CF 1033** (`api.` / `app.` / `ops.`; Lookout + Lead **~16:36**; **was CLEAR**). Vault independently **OPEN** subclass **530 / CF 1033** connector-down (Lead-verified **~16:28** America/Edmonton; **was 502** since ~19:45 MT Sep 13) on `/alive` **and** `/api/config` — **not** CLEAR. Soft-530 **OPEN 1033** + vault **1033** is dual OPEN (half-state **OVER**). Recovery fork: **1033** → lightning Healthy / cloudflared first; **502** → `docker.service` enabled+active. Subclass flip = Lead/Lookout field DIFF ≠ Ben re-nag if already asked once. ListMachines **EMPTY** / Docs **ABSENT** (Lead-verified **~16:51** after flap-while-connected); lightning **ABSENT** (McKing sleep intentional); `Mac.lan` may CONNECTED — `Mac.lan` ≠ `Docs-MacBook-Pro` ≠ `lightning` (living is still **home/lab / pre-cutover**; Doc awake at home = valid pre-camp half-state; camp cutover paper **ready, pending Ben GO**). Soft-530 CLEAR + vault OPEN ≠ dual CLEAR — **do not enter** [post-dual-clear-go.md](post-dual-clear-go.md) until vault CLEAR. Soft-530 CLEAR at home ≠ camp GO. Soft-530 **CLEAR ≠ #82** ≠ unfreeze ≠ Bitwarden. Lookout: api `ok=false` **expected**; vault `ok=false` **expected** (dual OPEN both 1033). Option A Redirect **PASS**. Brochure Option A **LIVE** (FULL 10/10; `waitlist.js?v=3`; `styles.css?v=36`; Home bare `href="/"` + canonical/og apex until **#82**). Freeze last_known **`4cf8924`** / **`5swmVz`** — live-verify **now allowed**, still **unverified**. **#82** still Ben GO. Chief checking lightning cloudflared/tunnel separately.
