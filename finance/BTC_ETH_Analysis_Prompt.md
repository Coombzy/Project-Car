...
**Version:** 1.10  
**Last edited:** 2026-09-06T15:20:00Z  
...
2. **Regime** per asset: `trend-up` | `trend-down` | `digestion` | `failed-break`.
   ...
   - **Takeaway word-ban (v1.9):** if `pred_regime` is `trend-up`, the Key Takeaway and title must **not** contain the word `digestion` (Sep 5 takeaway: “digestion risk remains elevated” while table said trend-up). Sep 6 title/takeaway complied.
...
14. **Write-streak emergency (v1.10).** If tracker Last audit documents ≥3 consecutive Analysis write misses, the only tools after the two GitHub reads + one quote batch are get-SHA → `create_or_update_file` → if that fails, `push_files` → verify. If Last audit documents ≥5 consecutive misses (now **8**: Aug 29 75s, Aug 31 74s, Sep 1 227s, Sep 2 323s, Sep 3 311s, Sep 4 54s, Sep 5 39.6s, Sep 6 36.7s), emit the table then **only** GitHub write tools — no snapshot, no narrative, no extra search. A complete table in the email without a 40-character hex `TRACKER_SHA` is still a failed run (Sep 4 54s, Sep 5 39.6s, and Sep 6 36.7s had the table).
...
- **Applies on weekdays.** Short runtime is not a skip. (Rule 11–14.) **37s is not a skip. 40s is not a skip. 54s is not a skip. 311s is not a skip. 323s is not a skip.**
...
- **Post-write verify:** re-get the tracker and confirm today's 8 `(analysis_date, asset, horizon)` rows exist. If absent after one retry of each write tool, paste the 8 rows in the response and write `WRITE FAILED` — do not mark the run successful.
