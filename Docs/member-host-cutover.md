...
**Ben GO required** before Garage (site) or Zone (Cloudflare DNS / tunnel / CORS edge) start this cutover — **and** only after **#82** Worker-live + Bulk Phase1 frees pretty-URL Dynamic slots (`member-zone-edge.md` §0). Option A **FULL 10/10** capacity-blocks `/member*` ranking + www→apex `/member*` 301. **#83** is CI-only on the **#82** base (thin `_redirects` assert) — prefer merge after **#82**; **parallel with upload OK**; does **not** need Bulk Phase1. **Never #81.** Bulk Phase1 stays after the **#82** upload **only** for these Member slots. Cutover planning outranks new breadth placeholders. Do **not** execute from this file.
