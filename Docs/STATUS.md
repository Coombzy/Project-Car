...
- **Brochure P2-3:** cache `?v=` SSOT landed in git (PR **#57** / `be60a01`: `styles.css?v=34` + `banner-logo.png?v=30` on all public HTML). Git/`rg` verify only — Zone owns the next Direct Upload; do **not** invent Worker-live.
...
Two gates — do **not** collapse them into “v1 includes everything forever.” Calendar (#18), fill (#20), placeholders (#21), cookies (#22), Docs (#23 / #25 / **#30–#57**, website **#52–#57**), schedule harden (#24), inventory (#26), Chat (#27), Dashboard (#28), shop-web host allowlist (**#36**, `f952cd3`), brochure P1-1 / P1-3 (**#52**, `5361212`), brochure P2-1 (**#55**, `453e44d`), brochure P2-2 (**#56**, `ed41d23`), and brochure P2-3 (**#57**, `be60a01`) are **on `main`** — they are not Next. P2-2 is **LIVE** on Worker. P2-3 cache `?v=` SSOT is on git until Zone upload (not Worker-live). P2-4 stays **open** (this PR is the Zone checklist only — not applied). **#36 is on `main` only** until Doc pull + rebuild. **`ops.projectcar.ca` is LIVE** at the edge. `app.` remains the temporary alias — do not cut it here.
