...
**Version:** 1.15  
**Last edited:** 2026-09-13T16:20:00Z  
...
4. **Volume / confidence**
   - Buckets vs 20d: `<0.80x` below-avg; `0.80–1.19x` normal; `>=1.20x` elevated. Same value on all four rows. **2026-09-10** Vol ~118.60M / ~$82.0M ≈ 1.45x = elevated. **2026-09-11** Vol ~79.27M / Barchart 20d $79.97M ≈ 0.99x = **normal**. Monday 9/14 analysis must use 9/11 as last completed — do not copy Friday `pred_rel_vol=elevated`.
...
17. **Write-streak emergency ON (v1.15).** Current streak: **3** (9/09 + 9/10 + 9/11 session-day misses). **Streak-clear = session-day 4-row INSERT + post-write SHA-delta only.** Weekend path-refresh / SHA-reuse does not clear or increment: 9/12 commit 572a064 / audit 5ffa6f02; 9/13 Daily title “path-refresh complete” reused blob 536ccee1 / tree 58aafc61. Streak remains 3 until next session day (2026-09-14) lands a new 40-char blob SHA with that day's four rows verified on post-write get_file_contents. Do not pre-insert 2026-09-14 rows (Daily inserts session-day rows).
...
Never print `write pending`. Write-streak emergency is ON (v1.15). Streak = 3 until a session-day 4-row insert + SHA-delta. Weekend path-refresh / SHA-reuse is not a clear. Dual-write + SHA-delta + payload-compact remain. Pre-write SHA emission = WRITE FAILED.
