...
Soft-530 **CLEAR** **`soft530_clear_since` ~07:25** (Lead `api.` `/health` **200** ok; `app.` + `ops.` **307**). **`soft530_open_blip` ~07:06–~07:25**. Prior overnight CLEAR dwell **~22:00–07:06** (~9h) **ENDED** at the blip. Vault still **OPEN 530 / CF 1033** — **EXPECTED**. ListMachines **EMPTY**. **CONNECTED ≠ Shellable ≠ tunnel healthy.** Camp / [post-dual-clear-go.md](post-dual-clear-go.md) **must not reopen** on this recover. Re-offer camp vault-on-Doc Ben GO only after Docs **CONNECTED+Shellable** + clear-smoke + ≥30m SAME-CLASS dwell. Shared OPEN-since-**~07:06** queue **cancelled**. Option A Redirect+UX **PASS**. Home **STALE** until **#82**. Freeze **`last_known` `4cf8924` / `5swmVz`** (behind intact). **`shared_fleet_card_stale=true`**. Half-state ≠ dual CLEAR. ≠ auto camp / **#82** / **#79.1** / unfreeze / Bitwarden.
