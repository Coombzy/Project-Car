...
- `brochure-worker-deploy.md` — standing Zone Direct Upload of `apps/website/html` onto Worker `projectcar-brochure`. Option A Worker-live smoke gate: Home/nav `href="index.html"`; canonical + `og:url` `https://projectcar.ca/index.html`; Worker `_redirects` thin chat-only. Zone 301 pretty-URL pack alone ≠ Option A HTML finish. merge **#82** ≠ Worker-live until smoke **PASS**.
...
| Date | Change |
|------|--------|
| 2026-09-21 | Pointer only: Option A Worker-live smoke gate lives in `brochure-worker-deploy.md`. Reality tip / Option A finish cannot claim live HTML done until smoke **PASS** (Home/nav `href="index.html"`; canonical + `og:url` `https://projectcar.ca/index.html`; Worker `_redirects` thin chat-only). Zone 301 pack alone ≠ HTML finish. merge **#82** ≠ Worker-live. Soft-530 **OPEN** `home_expected` is quiet-ops, not an incident stamp. No live-audit rewrite in this row. |
