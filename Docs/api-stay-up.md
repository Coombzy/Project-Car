...
Alerts can come from anyone who sees a **502**, **530 / error 1033**, or a failed waitlist submit. **Recovery of the process is Lead only.** Lookout owns the live api watch (`enabled:true`). Soft-530 companion watches (`ops.` / `app.` `/login`) stay **pending Ben GO** — do **not** claim armed. Do not instruct Garage (or anyone else) to restart uvicorn. Companion flip alerts: **Chief + Lead only**; **never Ben**; never restart / mutate.
