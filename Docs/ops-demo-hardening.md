...
**Status:** Plan only — Soft-530-CLEAR **Next Ben GO**. **Not** a GO from this tip-fold. Do **not** change login HTML or enable Cloudflare Access here.  
**Updated:** 2026-09-11 (elevated on held **#70**)  
...
| **Doc freeze unchanged** | Doc checkout stays **`4cf8924`** / BUILD_ID **`5swmVz`** until the **separate** unfreeze GO (`doc-unfreeze.md`). A merged Garage login PR is **git-only** until that pull + rebuild. Hardening GO ≠ unfreeze GO. |
| **No #82 / Option A / Bulk fan-out** | Do **not** execute or rewrite **#82** Worker-live, Option A **FULL 10/10**, or Bulk Phase1 from this card. |
...
**Soft-530 CLEAR re-exposed the public demo password.** Lead/Chief verified live: `https://ops.projectcar.ca/login` (and the `app.` alias) still shows banner Password `` `changeme` `` and **prefills** the password input. Same-class leak on `/member/login`. Email prefill is **ok**. That is why **#79** is now Soft-530-CLEAR **Next Ben GO** — more urgent than when Doc was 530.
...
|---------|----------------|
| **`https://ops.projectcar.ca/login`** | **200** when Doc origin is up (Lead/Chief live). HTML prints demo Owner email **and** plaintext password **`changeme`** (`apps/project-car/web/app/login/page.tsx` banner + `login-form.tsx` `defaultValue`). Frozen Doc tip **`4cf8924`** and later git tips still ship that print/prefill. |
...
| **Probes** | Lookout `projectcar-api-health-watch` is **resumed** (`enabled:true`) 2026-09-11 ~06:52 America/Edmonton (Lookout confirmed; live `/health` **200**). Lead interim probe **ended**. **#78 lookout-resume exception:** freeze does **not** block that re-arm. Do **not** re-pause or re-arm from this file. |
...
### Garage implement notes (later PR — **not now**, **not this tip-fold**)

1. **Ben GO** to strip the print (this file is the elevated plan, not that GO).
...
4. **Lead** `git pull` + `npm run build` + `next start` only after **Ben GO** unfreeze (`doc-unfreeze.md`). Until that rebuild, public `/login` HTML on Doc is still the old banner.
...
Do **not** flip Access from the Garage PR. Do **not** cut `app.`. Do **not** start that Garage PR from this **#70** fold.
...
| **#79** | Soft-530-CLEAR **Next Ben GO** | This card. Plan PR may still be open; living-ops / Next now live here. |
| **#78** | Lookout re-arm | **Done** (`enabled:true`). Do **not** flip `enabled` from here. |
| **#82** / Option A / Bulk Phase1 | Worker-live + capacity | **Do not touch.** Not this fold. Option A stays **FULL 10/10**. **#82** not yet Worker-live. |
| Garage §1 | Stop printing password | **Ben GO.** Git PR, then Doc rebuild only after **separate** unfreeze GO. |
...
- Lookout `enabled` flip (already resumed).
...
- Soft-530 **CLEAR** re-exposed the leak; that is **Next Ben GO**, not “GO now.”
...
- `STATUS.md` living-ops / Next (Soft-530-CLEAR **Next Ben GO**)
