...
- **Brochure P2-1:** **LIVE** on Worker `projectcar-brochure` (2026-09-07). Garage PR #55 → `453e44d`. Absolute `og:url` / `og:image` / `twitter:image` at `https://projectcar.ca/...` plus `og:type` + `twitter:card`. Zone smoke PASS.
- **Brochure P2-2:** apex `<link rel="canonical">` on every public HTML page is **in git** this PR (paths match P2-1 `og:url`; host `https://projectcar.ca`, not www). Zone owns Worker upload after merge — do **not** claim P2-2 live until that upload. P2-3 / P2-4 stay open.
...
Two gates — do **not** collapse them into “v1 includes everything forever.” Calendar (#18), fill (#20), placeholders (#21), cookies (#22), Docs (#23 / #25 / **#30–#55**), schedule harden (#24), inventory (#26), Chat (#27), Dashboard (#28), shop-web host allowlist (**#36**, `f952cd3`), brochure P1-1 / P1-3 (**#52**, `5361212`), and brochure P2-1 (**#55**, `453e44d`) are **on `main`** — they are not Next. P2-2 canonicals ship in this PR (git; live after Zone upload). **#36 is on `main` only** until Doc pull + rebuild. **`ops.projectcar.ca` is LIVE** at the edge. `app.` remains the temporary alias — do not cut it here.
