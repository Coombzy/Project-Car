...
- **Ops management UI (LIVE at edge):** **`https://ops.projectcar.ca` is LIVE** (2026-09-06 ~12:55 America/Edmonton) **when Doc origin is up**. Zone Cloudflare tunnel v8 hostname `ops` → `http://127.0.0.1:3000` (same Doc shop UI as `app.`). KeepAlive `com.projectcar.shop-web` runs **`next start`** (not `next dev`) on Doc. Shop-web **BUILD** after Dashboard **#28** (`main` `afb37f9`, `BUILD_ID` `5swmVz-T2CqKEQzTk1ifU`). Tunnel origin preferred `http://127.0.0.1:3000` (not bare localhost) after IPv6 `[::1]` 502. Public probes **when Doc origin is up**: `/` → `Location: https://ops.projectcar.ca/login` (no localhost hop); `/login` **200**. Lid-close / sleep (mornings included) still kills the Mac; the public edge then returns Cloudflare **502** or **530 / error 1033** — tunnel / lid-close, not a new product break. Lookout owns the live probe (same class as `projectcar-api-health-watch`). CORS includes `https://ops.projectcar.ca`. Some clients still have **flaky local DNS cache** for `ops.` — **use the `app.` alias**. Still the Doc demo — not a shop opening. Stay-up / rebuild / 1033-vs-flap: [shop-web-stay-up.md](shop-web-stay-up.md).
