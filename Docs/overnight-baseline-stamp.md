...
2. **Living morning-refresh stamp** after first-hop absorb (STATUS pointer / example, **not** a live ops action). Overnight `lightning` DIFF ~19:06 / ~19:10 MT Mon 2026-09-14 was handled by [mcking-reappear-first-hop.md](mcking-reappear-first-hop.md). Morning first `*/20` **must not** re-surface that DIFF to Ben (hop completed + Ben already asked once + Soft-530 / vault HTTP class unchanged). **Refresh** the living stamp so later same-day `*/20` is same-class quiet (`611c267`). Optional short Lead-only ack: stamp refresh + McKing first-hop pointer. Still blocked on **Ben sudo**. Vault still **OPEN 502** (docker dead) — **not** CLEAR. Soft-530 still **OPEN 1033**. Docs **ABSENT** — **home-off expected** ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)); Soft-530 **OPEN** + Docs **ABSENT** is **SAME-CLASS expected**, not a morning Ben DIFF. Quiet-ops **HOLD**. **Honesty ~12:24:** `Mac.lan` dropped; only **`lightning` CONNECTED**. **Honesty ~12:45:** `Mac.lan` flap **reappeared**; **`Mac.lan` + `lightning`**; Docs still **ABSENT**; HTTP class unchanged — **not a Ben surface**.
...
**Living (this fold — Mac.lan flap reappear vs ~12:45; HTTP unchanged — not a Ben surface):**

```
STAMP 2026-09-15 ~12:45 refresh (paper / Mac.lan flap reappear) | Soft-530 OPEN api/app/ops/cloud 530/1033 waitlist_OPTIONS 530 since 2026-09-13T11:57 | vault OPEN /alive 502 /api/config 502 since 2026-09-13T19:45 (docker dead — not CLEAR) | ListMachines Mac.lan + lightning; Mac.lan flap reappeared vs ~12:45 (after ~12:24 drop); docs=ABSENT (McKing first-hop ran ~19:10 MT Mon 2026-09-14; still blocked on Ben sudo; HTTP class unchanged — not a Ben surface) | freeze last_known 4cf8924 / BUILD_ID 5swmVz / changeme (last Soft-530 CLEAR only; Soft-530 OPEN 530/1033 → unverifiable live — not a DIFF) | Option A Redirect PASS 10/10 apex+www; Soft-530 UX LIVE styles.css?v=36 + waitlist.js?v=3 + Discord honesty | Lookout api+vault chronic HOLD today (enabled:true + cron-dead after 1×; flip detection false); companions HOLD; brochure-option-a-redirect-watch GONE (overnight Dynamic UNCOVERED = accept) | locks=quiet-ops HOLD; Chronic Lookout ≠ quiet-ops failure ≠ Ben ping; #82 still Ben GO; no Ben re-nag; Zone vault. checklist paper only — no live execute
```

| Surface | Living (morning refresh after `lightning` first-hop absorb) |
|---------|--------------------------|
...
| **ListMachines** | **`Mac.lan` + `lightning`** — `Mac.lan` flap **reappeared** vs ~12:45 America/Edmonton 2026-09-15 (after the ~12:24 drop); `docs=ABSENT`. `Mac.lan` ≠ `Docs-MacBook-Pro` ≠ `lightning`. HTTP class unchanged (Soft-530 **1033** / vault **502**) — **not a Ben surface**. McKing first-hop already ran ~19:10 MT Mon 2026-09-14 — still blocked on Ben sudo. **No morning re-ask.** |
...
| **Overnight first-hop** | `lightning` DIFF ~19:06 / ~19:10 absorbed by [mcking-reappear-first-hop.md](mcking-reappear-first-hop.md). Vault HTTP class **unchanged** (still **502**). Soft-530 HTTP class **unchanged** (still **1033**). |
...
This paper does **not** invent CLEAR, overnight redirect smoke, a second Ben sudo / docker / Doc-wake / recreate ask, or a morning Ben re-page of an already-hopped ListMachines DIFF. Last-weekday Redirect **PASS 10/10** is the recorded weekday field — **not** invented overnight smoke.
