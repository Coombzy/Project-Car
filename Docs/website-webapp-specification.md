...
| 10 | Basic uptime / synthetic monitoring | **Done / partial** — Lookout owns **`projectcar-api-health-watch`** (live flips on `https://api.projectcar.ca/health`; 200 when Doc origin is up; lid-close 502/530). Brochure Option A watch **GONE** (deleted to fix stalled cron; Auto-review blocked recreate) — **HOLD** recreate; weekday Chief `*/20`; overnight/weekend Dynamic redirect **UNCOVERED**; SOP [lookout-rearm-sop.md](lookout-rearm-sop.md); spec [brochure-redirect-watch.md](brochure-redirect-watch.md) (flip Chief+Lead only when armed; **not** companion re-ask). **#78 lookout-resume** is **LIVE-SUPERSEDED**. Homepage-only 200 is insufficient. Apex dropped (deferred). Discord invite check optional/deferred. Do not invent extra shipped monitors beyond that paper. |
