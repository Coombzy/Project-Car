...
Living this fold — OPEN class honesty (do **not** invent CLEAR): Soft-530 is still **OPEN** as CF **1033** tunnel-down (`api.` / `ops.` / `app.` / `cloud.` **530** / error **1033**; waitlist OPTIONS **530**). Vault is independently **OPEN** as **502** origin-down — **not** 1033 — on `/alive` **and** `/api/config`. ListMachines shows only `Mac.lan` (Ben Laptop, LocalHostName Laptop; `:8000`/`:8080` down) — `Docs-MacBook-Pro` + `lightning` absent. Different first checks when `lightning` returns: Soft-530 **1033** → Doc tunnel (this file); vault **502** → VW origin ([vault-stay-up.md](vault-stay-up.md)). Brochure Option A + Soft-530 Discord assets stay **LIVE**. **#82** unchanged (**never #81**). This paper does **not** invent a CLEAR or a live restore. Capture on **Doc** (`Docs-MacBook-Pro`) after the **next** lid-restore CLEAR — **not** on `Mac.lan`. Vault twin (McKing forensics) is [vault-stay-up.md](vault-stay-up.md) **Post-CLEAR stay-up evidence**.
