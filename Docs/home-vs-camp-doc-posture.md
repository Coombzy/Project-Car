...
| **HOME-awake CLEAR** (superseded ~16:36) | Docs Mac ListMachines **CONNECTED** — Shell **unreachable**; lid-restore **blocked** | Living **OPEN 530 / CF 1033** (`api.` / `app.` / `ops.`; Lookout + Lead **~16:36**; **was CLEAR**) | Independently **OPEN 530 / CF 1033** connector-down (was 502) | **Dual OPEN both 1033.** Half-state **OVER**. **Do not** enter [post-dual-clear-go.md](post-dual-clear-go.md). **Do not** treat as camp GO. Ben asked once Doc awake/desktop — **no re-nag**. Lookout api `ok=false` / vault `ok=false` **expected**. Home lid-open CLEAR **≠** [camp CDM soak PASS](camp-vault-on-doc-cutover.md#camp-cdm-lid-close-soak-gate). |
...
| **Lookout Soft-530** | Home-off: `ok=false` / non-200 `/health` while Docs **ABSENT** = **expected baseline**. Living dual OPEN: api `ok=false` **expected**; vault `ok=false` **expected**. Half-state (api `ok=true` / vault `ok=false`) is **class**, not tonight. Stay armed. Do **not** re-arm Soft-530 companions. | Stickiness panic. Companion re-ask. Treat expected `ok=false` (home-off api, living dual OPEN, or vault in half-state) as a new stall incident that pages Ben. |
