...
While `brochure-option-a-redirect-watch` is **GONE**, overnight (this card) **and** weekend (`84107e7` stay-up rows) Option A Dynamic redirect coverage is **explicitly uncovered — accept that risk**. A **weekday** `*/20` gap **>~40m** while Soft-530 is **OPEN** is a **different** hole — **mid-weekday UNCOVERED**, not this overnight card ([plan-improve-weekday-coverage-hole.md](plan-improve-weekday-coverage-hole.md)). Do **not** invent overnight plan-improve, Zone, or Garage coverage. **Next Option A smoke** = the recovering plan-improve fire **or** Ben-approved Lookout recreate ([lookout-rearm-sop.md](lookout-rearm-sop.md)). Cross-link: [lookout-rearm-sop.md](lookout-rearm-sop.md) · this file · [plan-improve-weekday-coverage-hole.md](plan-improve-weekday-coverage-hole.md) · weekend Soft-530 coverage (`84107e7` / stay-up rows). **HOLD** a second Ben recreate ask (`lookout-rearm-sop` already one-ask).

Last weekday `*/20` writes the [overnight-baseline-stamp.md](overnight-baseline-stamp.md) one-liner **before** this window — **refresh only if any field diffs**; else the prior stamp **stands**. Tonight **~22:00** stamped Soft-530 **CLEAR** + vault **OPEN 530 / CF 1033** as **half-state** (first Mon–Thu overnight after Sep 13–15 recovery; ~17:20 dual OPEN is **historical**). That CLEAR stamp is **superseded ~07:06**, then the OPEN flap **recovered ~07:25**. Living **`soft530_clear_since` ~07:25** + **`soft530_open_blip` ~07:06–~07:25**. Prior overnight CLEAR dwell **~22:00–07:06** **ENDED** at the blip. Living vault subclass (Lead-verified **~16:28**) is **530 / CF 1033** connector-down — **was 502** — **EXPECTED**. Soft-530 **CLEAR** + vault **1033** is **half-state** — **≠ dual CLEAR**. Camp / post-dual-clear-go **must not reopen** while Docs **EMPTY** ([soft-530-blip-recover.md](soft-530-blip-recover.md)). **CONNECTED ≠ Shellable ≠ tunnel healthy.** First weekday `*/20` after ~06:00 diffs that stamp instead of rediscovering. **Same-class = stay quiet** (`611c267`). Surface **Lead/Ben only on stamp class DIFF** (Soft-530 / vault **incl. subclass** / ListMachines / Option A / freeze / Lookout). **Same half-state ≠ dual CLEAR ≠ “incident closed.”** Soft-530 **CLEAR→OPEN** = DIFF (Lead wake / [doc-reappear-first-hop.md](doc-reappear-first-hop.md)); vault still **530 / CF 1033** = **SAME** for vault; vault **502 ↔ 530/1033** = Lead/Lookout field DIFF (**not** Ben re-nag if already asked once); Docs **CONNECTED→ABSENT** = DIFF; freeze `changeme` flipping **off** = DIFF (unfreeze signal). If a ListMachines DIFF was **already first-hop handled** overnight, morning **must not** re-surface Ben — **refresh** the living stamp. Soft-530 **CLEAR** overnight **≠** camp CDM soak **PASS** **≠** companions re-arm **≠** **#82** **≠** unfreeze **≠** Zone vault retarget **≠** Bitwarden auto. Friday last `*/20` stamps, then hands **into weekend coverage**. Stamp alone ≠ Ben re-nag ≠ Zone / Garage. Overnight **CLEAR** still [soft-530-clear-smoke.md](soft-530-clear-smoke.md) / [vault-clear-smoke.md](vault-clear-smoke.md) + first-hop cards.
...
STATUS pointers stay canonical. Honesty this fold — **half-state** Soft-530 **CLEAR** + vault **OPEN** (first Mon–Thu overnight after Sep 13–15 recovery; **`soft530_clear_since` ~22:00**):

| Surface | Living |
|---------|--------|
| **Soft-530** | **CLEAR** — `api.` `/health` **200** **`soft530_clear_since` ~22:00** (Lookout + Lead; same-day OPEN dwell **~16:36–22:00**). Do **not** keep the ~17:20 OPEN stamp. |
...
| **ListMachines** | ListMachines Docs `95a229f5` **CONNECTED+Shellable** (KeepAlive **UP** ~22:07–22:08; EMPTY flap **~16:51–22:00** ended). **CLEAR ≠ ABSENT.** Companions **HOLD** forensics. Lightning **ABSENT**. `Mac.lan` may CONNECTED. `Mac.lan` ≠ `Docs-MacBook-Pro` ≠ `lightning`. Ben asked once Doc awake/desktop — **no re-nag**. **Not** camp GO. |
| **Freeze** | Live-verify **now allowed**. `app.` + `ops.` `/login` **307** **PASS**. **`last_known`** `` `changeme` `` / BUILD_ID **`5swmVz`** / HEAD **`4cf8924`**. |
| **Combined class** | Soft-530 **CLEAR** + vault **OPEN 530 / CF 1033** = **half-state**. ≠ dual CLEAR ≠ “incident closed.” Soft-530 still CLEAR overnight = **SAME**. Vault still **530 / CF 1033** = **SAME**. Vault **502 ↔ 530/1033** = Lead/Lookout field DIFF ≠ Ben re-nag. Lightning still **ABSENT** = SAME (McKing sleep intentional). |
| **#82** | Still **Ben GO**. Soft-530 **CLEAR ≠ #82**. Soft-530 **OPEN ≠ #82 blocked**. Home **STALE** until **#82**. This overnight gap still ≠ auto-GO |
| **Lookout** | api `ok=true` **expected**; vault `ok=false` **expected** (half-state). Brochure redirect `*/5` watch **GONE** — interim = plan-improve `*/20`. Companions **HOLD**. |
...
| **Bitwarden** | Import/rotate **blocked** until vault **CLEAR**, then still **Ben GO**. Overnight **530 / CF 1033** ≠ rotate. |
