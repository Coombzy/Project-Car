...
| **Vault** | Public **OPEN subclass 530 / CF 1033** connector-down (Lead-verified **~16:28**; **was 502** since ~19:45 MT Sep 13 — **not** CLEAR). First check: lightning Healthy / cloudflared. |
| **ListMachines** | **Docs + lightning CONNECTED**. `Mac.lan` may flap. `Mac.lan` ≠ `Docs-MacBook-Pro` ≠ `lightning`. Home-awake pre-camp **half-state** — **not** camp GO. McKing first-hop already ran ~19:10 — still blocked on Ben sudo. **No morning re-ask.** |
...
| **Combined class** | Soft-530 **CLEAR** + vault **OPEN 530 / CF 1033** = **first-class half-state**. ≠ dual OPEN ≠ dual CLEAR ≠ “incident closed.” Soft-530 **CLEAR** + vault **1033** is still half-state. Soft-530 **CLEAR→OPEN** overnight = Lead wake / [doc-reappear-first-hop.md](doc-reappear-first-hop.md). Vault still **530 / CF 1033** = **SAME**. Vault **502 ↔ 530/1033** = Lead/Lookout field DIFF ≠ Ben re-nag. Docs **CONNECTED→ABSENT** = DIFF. |
