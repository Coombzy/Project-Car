...
| **HOME-awake OPEN flap** (living ~07:06 / CONNECTED+Shell-unreachable) | Docs Mac `95a229f5` **CONNECTED** / Shell **unreachable** | Living **OPEN** (`api.`/`app.`/`ops.` **530 / CF 1033**; **`soft530_open_since` ~07:06**; prior CLEAR dwell **~22:00–07:06** ended) | Independently **OPEN 530 / CF 1033** connector-down (was 502) | **Dual OPEN both 1033.** **CONNECTED ≠ Shellable ≠ tunnel healthy.** **Do not** enter [post-dual-clear-go.md](post-dual-clear-go.md). GO window **paused**. **Do not** treat as camp GO. Ben asked once Doc awake/desktop — **no re-nag** / **≠ auto lid-restore nag**. Lookout api `ok=false` / vault `ok=false` **expected**. |
