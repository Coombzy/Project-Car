...
- **Brochure P2-3:** cache `?v=` SSOT landed in git this PR (`styles.css?v=34` + `banner-logo.png?v=30` on all public HTML). Git/`rg` verify only — Zone owns the next Direct Upload; do **not** invent Worker-live. P2-4 stays **open**.
...
Two gates — do **not** collapse them into “v1 includes everything forever.” Calendar (#18), fill (#20), placeholders (#21), cookies (#22), Docs (#23 / #25 / **#30–#56**, website **#52–#56**), schedule harden (#24), inventory (#26), Chat (#27), Dashboard (#28), shop-web host allowlist (**#36**, `f952cd3`), brochure P1-1 / P1-3 (**#52**, `5361212`), brochure P2-1 (**#55**, `453e44d`), and brochure P2-2 (**#56**, `ed41d23`) are **on `main`** — they are not Next. P2-2 is **LIVE** on Worker. P2-3 cache `?v=` SSOT lands in this PR (git/`rg` verify; not Worker-live). P2-4 stays open. **#36 is on `main` only** until Doc pull + rebuild. **`ops.projectcar.ca` is LIVE** at the edge. `app.` remains the temporary alias — do not cut it here.
