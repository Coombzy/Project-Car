...
Last weekday `*/20` (~17:40 MT) writes the [overnight-baseline-stamp.md](overnight-baseline-stamp.md) one-liner **before** this window — **refresh only if any field diffs**; else the prior stamp **stands**. First weekday `*/20` after ~06:00 diffs that stamp instead of rediscovering. **Same-class = stay quiet** (`611c267`). Surface **Lead/Ben only on stamp class DIFF** (Soft-530 / vault / ListMachines / Option A / freeze / Lookout). If a ListMachines DIFF was **already first-hop handled** overnight, morning **must not** re-surface Ben — **refresh** the living stamp ([overnight-baseline-stamp.md](overnight-baseline-stamp.md)). **Couldn’t verify freeze tip / changeme / BUILD_ID while Soft-530 is OPEN (530/1033) is NOT a freeze DIFF** — those fields stay `last_known` from the last Soft-530 CLEAR; live-verify waits on [soft-530-clear-smoke.md](soft-530-clear-smoke.md). True-last `*/20` **~17:42** MT Mon 2026-09-14 was **SAME-CLASS** vs the ~17:25 stamp — **do NOT refresh**. Friday last `*/20` stamps, then hands **into weekend coverage**. Stamp alone ≠ Ben re-nag ≠ Zone / Garage. Overnight **CLEAR** still [soft-530-clear-smoke.md](soft-530-clear-smoke.md) / [vault-clear-smoke.md](vault-clear-smoke.md) + first-hop cards.
...
| **Vault** | `/alive` **502** + `/api/config` **502** since ~19:45 MT Sep 13 (**not** 1033; docker dead — **not** CLEAR) |
| **ListMachines** | **`Mac.lan` + `lightning`** — `docs=ABSENT`. `Mac.lan` ≠ `Docs-MacBook-Pro` ≠ `lightning`. McKing first-hop already ran ~19:10 — still blocked on Ben sudo. **No morning re-ask.** |
...
This paper does **not** invent CLEAR, overnight redirect smoke, or a morning Ben re-ask of the already-hopped `lightning` DIFF. Docs **ABSENT** + Soft-530 **OPEN** is **home-off expected** ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)) — not a mid-overnight incident.
