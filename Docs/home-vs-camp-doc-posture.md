...
| **HOME-awake CLEAR** (living ~22:00) | Docs Mac ListMachines **EMPTY** / **ABSENT**; lid-restore **parked** | Living **CLEAR** (`api.` `/health` **200**; **`soft530_clear_since` ~22:00**; same-day OPEN dwell **~16:36–22:00**) | Independently **OPEN 530 / CF 1033** connector-down (was 502) | **Half-state.** **CLEAR ≠ auto Doc wake.** **Do not** enter [post-dual-clear-go.md](post-dual-clear-go.md). **Do not** treat as camp GO. Ben asked once Doc awake/desktop — **no re-nag**. Lookout api `ok=true` / vault `ok=false` **expected**. Home lid-open CLEAR **≠** [camp CDM soak PASS](camp-vault-on-doc-cutover.md#camp-cdm-lid-close-soak-gate). |
