...
| Freeze live-verify **PASS** — **both** **`freeze_git_verified=true`** (Doc checkout `/Users/dochak/src/Project-Car` SHA **`4cf8924`** / disk BUILD_ID **`5swmVz-T2CqKEQzTk1ifU`** / behind **89**) **and** **`freeze_build_id_public`**. Living **~08:41**: Soft-530 **OPEN** → login **530** — public scrape **unreachable**. **`last_known` `freeze_build_id_public=true`** (~08:16 MATCH). **`freeze_git_verified`** still **queued**. Last_known MATCH **≠** this PASS. `` `changeme` `` print is expected HTML — **not** the verify | Unfreeze. Live-verify **PASS** = freeze **confirmed**, not lifted. SHA≠`4cf8924` or BUILD_ID≠`5swmVz*` → freeze **DRIFT** (Lead) — still ≠ GO. |
...
[soft-530-clear-smoke.md](soft-530-clear-smoke.md) already ran (or its five-row class is living **CLEAR**). Freeze live-verify is **now allowed** after that CLEAR. Living **~08:41** has **`freeze_build_id_public` last_known** only (OPEN → unreachable) — full **PASS** still needs a later CLEAR scrape **and** **`freeze_git_verified`** + Shellable. Those two + the behind-count unlock **drafting** only. Last_known MATCH **≠** unfreeze / **#82** / **#79.1**.
...
| **Freeze** | **`last_known` `freeze_build_id_public=true`** — public MATCH (~08:16). Living **OPEN ~08:41** → login **530** scrape **unreachable** (not a DIFF). **`freeze_git_verified`** still **queued** (last_known ~06:57 `true` / `4cf8924` / `5swmVz-T2CqKEQzTk1ifU` / behind **89**). Last_known MATCH **≠** live-verify **PASS**. [freeze-build-id-public-probe.md](freeze-build-id-public-probe.md) |
