...
Living this fold: Soft-530 is still **OPEN** (CF **1033** since 2026-09-13 ~11:57). Vault is independently **OPEN** (**502** since ~19:45 MT Sep 13 — **not** 1033; docker dead). ListMachines **`Mac.lan` + `lightning`** — `Docs-MacBook-Pro` is **ABSENT**. McKing first-hop already ran ~19:10 ([mcking-reappear-first-hop.md](mcking-reappear-first-hop.md)) — that ListMachines DIFF is stamp-absorbed; **no morning Doc-wake re-ask** from `lightning` being back. This hop still waits on `Docs-MacBook-Pro` + machineId **`95a229f5-9296-4a18-aa98-70fd300dabdf`**. This paper does **not** invent a Doc reappear or a CLEAR.
