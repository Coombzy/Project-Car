...
These match `OWNER_EMAIL` / `OWNER_PASSWORD` in the API `.env`. Click through Dashboard (hoist cards, today's bookings, token-at-risk), Schedule (week by hoist), Members, Hoists, Waitlist (mark contacted), and Tiers (edit allowances).
