...
| **Home half-ops CLEAR ≠ missed camp GO** | Soft-530 **CLEAR** while Docs **ABSENT** is **home half-ops living**. It is **not** a missed camp GO and **not** a Shared amend failure. Shared cancel of a queued CLEAR write is **correct**. Only stamp `missed_clear_window` when Ben had an **explicit camp dual-CLEAR GO armed** and Shellable never arrived. Today's **~07:25–~08:41** EMPTY CLEAR was this class — **do not** carry missed-GO debt into STATUS Reality. |
...
Living this fold (America/Edmonton **~12:29** 2026-09-16 — do **not** invent dual CLEAR or a live retarget): Soft-530 **OPEN** SAME-CLASS ~**3.8h** — Lookout `api.` **200→530** CF **1033**; Lead `api.`/`app.`/`ops.` all **530/1033** (**`soft530_open_since` ~08:41**; prior **home half-ops** CLEAR dwell **~07:25–~08:41** ended; **`soft530_open_blip` ~07:06–~07:25**). Vault independently **OPEN 530 / CF 1033** connector-down (was 502 since ~19:45 MT Sep 13 on `/alive` + `/api/config` — **EXPECTED**, **not** CLEAR). ListMachines **Laptop.local only** (`Mac.lan` ≠ Docs); **EMPTY→Laptop-only is non-progress**; **CONNECTED ≠ Shellable ≠ tunnel healthy**; companions **HOLD**; lightning **ABSENT** (McKing sleep intentional); **never** `Mac.lan` / **Laptop.local** as `Docs-MacBook-Pro`. Today's **~07:25–~08:41** EMPTY CLEAR was **home half-ops living** — **not** a missed camp GO / Shared amend failure. Shared **CLEAR since ~07:25** cancel was **correct**. **Do not** stamp `missed_clear_window`. **Do not** carry missed-GO debt into STATUS Reality. Living **OPEN ~08:41** — **not** camp GO reopen, **not** dual CLEAR ([soft-530-blip-recover.md](soft-530-blip-recover.md)). Ben asked once Doc awake/desktop — **no re-nag** / **≠ auto lid-restore nag**. Lookout: api `ok=false` **expected**; vault `ok=false` **expected** (dual OPEN). Brochure Option A Dynamic watch **GONE**. Option A Redirect **PASS**. Soft-530 UX **LIVE** (`styles.css?v=36` + `waitlist.js?v=3` + Discord honesty). Redirect+UX **PASS** ≠ Worker body freshness **PASS** / **#82** done (STATUS completeness ladder). Home **STALE** until **#82**. **`shared_fleet_card_stale=true`**. Freeze **`last_known`** `` `changeme` `` / **`5swmVz`** / **`4cf8924`** (Soft-530 **OPEN** → unverifiable ≠ DIFF). **#82** still Ben GO — **no re-nag**. Soft-530 **CLEAR ≠ #82** ≠ unfreeze ≠ Bitwarden. Quiet-ops. Camp cutover paper **ready, pending Ben GO** — Zone `vault.` checklist is hop **4** on [camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md) (**not** a live retarget). **Doc local VW/NC healthy ≠ public vault CLEAR** ([camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md) scoring): Doc NC (`:8080`) / VW (`:8222` `/alive`) loopback **200** + compose healthy after cold-boot while public `vault.projectcar.ca` still **502** on McKing = **Z1 preflight only** — still vault **OPEN** (not partial CLEAR / not dual CLEAR / not camp progress %). [vault-clear-smoke.md](vault-clear-smoke.md) SSOT stays the public hostname; **never** substitute Doc loopback. Soft-530 **CLEAR** alone still **≠** camp GO. Soft-530 **CLEAR** lid-open / Doc awake at home **never scores** the [camp CDM lid-close soak](camp-vault-on-doc-cutover.md#camp-cdm-lid-close-soak-gate). This paper does **not** page Ben for Doc-wake or McKing docker. **No** Garage / Zone / Hatch fan-out. **No** companions re-ask.
...
| **HOME half-ops CLEAR** (historical ~07:25–~08:41 / EMPTY) | Docs Mac **ABSENT** / ListMachines **EMPTY** | **CLEAR** public HTTP (dwell met ≥30m) | Independently **OPEN 530 / CF 1033** | **Home half-ops living.** **Not** a missed camp GO. **Not** a Shared amend failure. Shared queued CLEAR amend **cancel** is **correct**. **Do not** stamp `missed_clear_window`. **Do not** enter [post-dual-clear-go.md](post-dual-clear-go.md). |
...
| **HOME-awake OPEN after home half-ops CLEAR** (living ~08:41 / Laptop.local only) | ListMachines **Laptop.local only** (`Mac.lan` ≠ Docs) | Living **OPEN** (`api.`/`app.`/`ops.` **530 / CF 1033**; **`soft530_open_since` ~08:41** SAME-CLASS ~**3.8h**; prior **home half-ops** CLEAR dwell **~07:25–~08:41** ended; **`soft530_open_blip` ~07:06–~07:25**) | Independently **OPEN 530 / CF 1033** connector-down (was 502) — **EXPECTED** | **Dual OPEN.** Quiet-ops. Lookout brochure **GONE**. **EMPTY ≠ cause of OPEN.** **Do not** enter [post-dual-clear-go.md](post-dual-clear-go.md). **Must not reopen** camp GO. Shared amend **HOLD**. Shared **CLEAR since ~07:25** cancel **stands** — **not** missed-GO debt. Re-offer only after next **CLEAR** + **CONNECTED+Shellable** + clear-smoke + ≥30m SAME-CLASS dwell ([soft-530-blip-recover.md](soft-530-blip-recover.md)). Ben asked once — **no re-nag**. ≠ auto lid-restore nag. Lookout api `ok=false` / vault `ok=false` **expected**. |
...
| **Home half-ops CLEAR = missed camp GO / Shared amend failure** | Soft-530 **CLEAR** while Docs **ABSENT** is **home half-ops living**. Shared cancel of **CLEAR since ~07:25** was **correct**. Only stamp `missed_clear_window` when camp dual-CLEAR GO was **armed** and Shellable never arrived. **Do not** carry `{start:~07:25,end:~08:41,reason:empty}` into STATUS Reality. |
| **#82 re-nag / companions re-ask / Garage-Zone-Hatch fan-out** | Quiet-ops. Home **STALE** until **#82** is honesty, not a nag. Companions **HOLD**. This fold does **not** assign or execute. |
...
- Carry missed-GO debt for **~07:25–~08:41** into STATUS Reality
