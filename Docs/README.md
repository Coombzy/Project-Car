...
| [brochure-worker-ci.md](brochure-worker-ci.md) | Paper CI + Option A **FULL 10/10** Member-precondition receipt (held **#70**). After **#82** GO: Direct Upload → **mandatory** Worker/Cache purge (apex+www) → **body freshness** (`HIT`+no-cache is steady-state; no-cache alone insufficient). Smoke: `waitlist.js?v=` + `styles.css?v=` + Soft-530 Discord fail-soft. **Never** Direct Upload from **#81** (draft + superseded). No new brochure pretty-URLs until Bulk Phase1 frees Member-edge slots. Paper only — **not** a Zone/Garage execute. |
