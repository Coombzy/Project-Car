...
| **Dashboard basics** | LIVE on Doc (#28). Git **#69** (`b9f9019`) is OwnerShell + bay-hero polish **on the keep path** (not a new breadth slice) — **git-only**; Doc checkout **frozen** at `4cf8924` until **Ben GO**. 24h bay strips (Bays 1–6), personal todos + ICS, parts PO strip. | **KEEP** | Demo IA already live. #69 is polish-on-keep-path, not a rewrite or new surface. Do not re-propose Doc pull. | No “Today’s bookings” list. Google / Apple Connect are stubs — two-way is not keep-path. |
...
| **`app.` alias removal** | Temporary alias still live. | **DEFER** | STATUS Next #2. **Ben** cuts that DNS. Plan: `app-alias-cut.md`. | This doc does **not** cut `app.`. Ops stay-up still documents the alias. Member cutover does **not** require the cut first. |
