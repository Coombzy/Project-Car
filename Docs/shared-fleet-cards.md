...
**Status:** Paper — Chief standing-notes hygiene. **Not SSOT.** Rewrite **DONE**. Living-host Soft-530 class+as-of **amend DONE ~22:08** (CLEAR — now **stale**). OPEN-since-**~07:06** queue **cancelled**. Shared **CLEAR since ~07:25** queue **cancelled** (home half-ops — **not** missed camp GO). Overnight stamp **`shared_fleet_card_stale=true`**.  
**Updated:** 2026-09-16 ~12:29 America/Edmonton  
**Related:** `STATUS.md` (Reality one-liner · living-ops / Locks), [deployment-guide.md](deployment-guide.md) (Monday paper map), [home-lab-specification.md](home-lab-specification.md) (host split), [camp-vault-on-doc-cutover.md](camp-vault-on-doc-cutover.md) (camp vault-on-doc), [unfreeze-readiness.md](unfreeze-readiness.md) (freeze / draft ≠ GO), [overnight-baseline-stamp.md](overnight-baseline-stamp.md) (morning Soft-530=**OPEN** + **`soft530_open_since` ~08:41** + **`missed_clear_window` omit** + ListMachines **Laptop.local only** + **`shared_fleet_card_stale=true`**; Shared never overrides Docs/#70 SSOT), [missed-clear-window-shared-supersede.md](missed-clear-window-shared-supersede.md) (`missed_clear_window` **only** when camp dual-CLEAR GO was armed; today's **~07:25–~08:41** = **home half-ops** — Shared cancel **correct**; do **not** carry missed-GO debt), [soft530-clear-vs-local-exec.md](soft530-clear-vs-local-exec.md) (Shared amend is **local-exec** — **HOLD** until next CLEAR + ≥30m + Shellable; **EMPTY ≠ cause of OPEN**), [soft-530-blip-recover.md](soft-530-blip-recover.md) (amend **only** after ≥30m SAME-CLASS CLEAR + Shellable — do **not** write OPEN-since-**~08:41** mid-flap), [soft-530-flap-while-connected.md](soft-530-flap-while-connected.md) (**CONNECTED ≠ Shellable ≠ tunnel healthy** — **never** `Mac.lan`)
...
| **as-of wall** | America/Edmonton wall time of that class. After same-day **CLEAR↔OPEN**, use the **current** flip — living **`soft530_open_since` ~08:41** (prior CLEAR dwell **~07:25–~08:41** ended; blip **~07:06–~07:25** noted). Do **not** keep the ~22:08 **CLEAR** as-of. Do **not** write the cancelled **OPEN since ~07:06**. Do **not** land OPEN-since-**~08:41** mid-flap. |
...
**Chief note:** Shared living-host **amend DONE ~22:08** (CLEAR). **CLEAR→OPEN ~07:06** re-queued an OPEN amend; **CLEAR recovered ~07:25** first → **cancel** that OPEN-since-**~07:06** queue. Shared **CLEAR since ~07:25** then **queued**. Living **CLEAR→OPEN again ~08:41** **before** Shellable — that CLEAR amend **never wrote**. **Cancel stands** — **home half-ops**, **not** a missed camp GO / Shared amend failure. **Do not** stamp `missed_clear_window` — no camp dual-CLEAR GO was armed. Do **not** leave stale “write CLEAR since ~07:25” across the flap. Do **not** backfill **CLEAR since ~07:25** as current living-host once OPEN. Shared amend stays **HOLD** until the **next** CLEAR recover + ≥30m SAME-CLASS + Docs **CONNECTED+Shellable**. Do **not** write OPEN-since-**~08:41** mid-flap. **`shared_fleet_card_stale=true`**. Shared still **never** overrides Docs/#70 SSOT. Essay: [missed-clear-window-shared-supersede.md](missed-clear-window-shared-supersede.md) · [home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md) · [soft530-clear-vs-local-exec.md](soft530-clear-vs-local-exec.md).
...
|-------------|--------------|
| **Soft-530 (Doc)** | **OPEN** since **~08:41** America/Edmonton SAME-CLASS ~**3.8h** — Lookout `api.` **200→530** CF **1033**; Lead `api.`/`app.`/`ops.` all **530/1033**. Prior **home half-ops** CLEAR dwell **~07:25–~08:41** **ENDED**. Same-day blip **~07:06–~07:25**. ListMachines **Laptop.local only** (`Mac.lan` ≠ Docs). **EMPTY→Laptop-only is non-progress.** **Never** treat laptop as Docs lid-restore. **EMPTY ≠ cause of OPEN.** Freeze **`last_known`**. Dual OPEN **living**. Quiet-ops. |
...
| **Shared `shop-os-mc-plan.md`** | **Rewritten** ~15:42; living-host **amend DONE ~22:08** on Doc `~/Desktop/Fleet-Nextcloud/Memory/Shared/shop-os-mc-plan.md`. Still **non-SSOT**. Living-host still: Soft-530 **CLEAR** since **~22:00** (stale). **CLEAR since ~07:25** queue **cancelled** — home half-ops; **not** missed camp GO. |
| **Overnight stamp** | Morning Soft-530=**OPEN** + **`soft530_open_since` ~08:41** + **`missed_clear_window` omit** + ListMachines **Laptop.local only** + **`shared_fleet_card_stale=true`** ([overnight-baseline-stamp.md](overnight-baseline-stamp.md) · [home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)). Shared still **non-SSOT**. Shared living-host still **CLEAR ~22:00** Sep 15 vs living **OPEN ~08:41** — amend waits Docs Shellable; **no backfill**. |
...
| **Combined class** | **Dual OPEN** Soft-530 **OPEN ~08:41** + vault **OPEN**. ≠ dual CLEAR ≠ camp GO reopen ≠ unfreeze ≠ **#82** auto ≠ auto lid-restore nag |
...
| **Don’t treat Shared as SSOT** | Amend **DONE ~22:08** is **stale** vs living **OPEN ~08:41**. OPEN-since-**~07:06** queue **cancelled**. **CLEAR since ~07:25** queue **cancelled** (home half-ops — **not** missed camp GO). Do **not** write OPEN-since-**~08:41** mid-flap. Do **not** backfill **CLEAR since ~07:25**. Overnight stamp **`shared_fleet_card_stale=true`**. Shared still **non-SSOT**. Docs/#70 is SSOT. |
...
| **Don’t keep “in progress” / don’t land cancelled queues** | The rewrite landed ~15:42. The living-host amend landed **~22:08**. The **~07:06** OPEN queue is **cancelled**. The **~07:25** CLEAR queue is **cancelled** (home half-ops). Do **not** write OPEN-since-**~08:41**. Do **not** backfill **CLEAR since ~07:25**. Next amend = next CLEAR recover + ≥30m + Shellable — **not** mid-flap. |
...
- Rewrite this morning’s overnight stamp away from Soft-530=**OPEN** + **`soft530_open_since` ~08:41** + prior home half-ops CLEAR dwell **~07:25–~08:41** ended + ListMachines **Laptop.local only** + **`shared_fleet_card_stale=true`** + **`missed_clear_window` omit**
- Land the cancelled Shared **OPEN since ~07:06** queue, land the cancelled **CLEAR since ~07:25** queue, write **OPEN since ~08:41** mid-flap, backfill **CLEAR since ~07:25** as current living-host, stamp `missed_clear_window` for home half-ops, or amend Shared before the next CLEAR recover + ≥30m SAME-CLASS + Shellable
