...
**Chief note:** Shared living-host **amend DONE ~22:08** (CLEAR). **CLEAR→OPEN ~07:06** re-queued an OPEN amend; **CLEAR recovered ~07:25** first → **cancel** that OPEN-since-**~07:06** queue. Living **CLEAR→OPEN again ~08:41**. Shared amend stays **HOLD** until the **next** CLEAR + ≥30m SAME-CLASS + Docs **CONNECTED+Shellable** ([soft-530-blip-recover.md](soft-530-blip-recover.md)). Do **not** write OPEN-since-**~08:41** mid-flap. **`shared_fleet_card_stale=true`**. Shared still **never** overrides Docs/#70 SSOT. Essay: [soft530-clear-vs-local-exec.md](soft530-clear-vs-local-exec.md) · [freeze-build-id-public-probe.md](freeze-build-id-public-probe.md).
...
| **Overnight stamp** | Morning Soft-530=**OPEN** + **`soft530_open_since` ~08:41** + prior CLEAR dwell **~07:25–~08:41** ended + ListMachines **EMPTY** + **`shared_fleet_card_stale=true`** ([overnight-baseline-stamp.md](overnight-baseline-stamp.md)). Shared still **non-SSOT**. |
...
| **Don’t treat Shared as SSOT** | Amend **DONE ~22:08** is **stale** vs living **OPEN ~08:41**. OPEN-since-**~07:06** queue **cancelled**. Do **not** write OPEN-since-**~08:41** mid-flap. Overnight stamp **`shared_fleet_card_stale=true`**. Shared still **non-SSOT**. Docs/#70 is SSOT. |
...
| **Don’t keep “in progress” / don’t land cancelled OPEN queue** | The rewrite landed ~15:42. The living-host amend landed **~22:08**. The **~07:06** OPEN queue is **cancelled**. Do **not** write OPEN-since-**~08:41**. Next amend = next CLEAR + ≥30m + Shellable — **not** mid-flap. |
...
- Land the cancelled Shared **OPEN since ~07:06** queue, write **OPEN since ~08:41** mid-flap, or amend Shared before the next CLEAR + ≥30m SAME-CLASS + Shellable
