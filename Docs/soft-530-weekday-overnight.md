...
STATUS pointers stay canonical. Honesty this fold — **dual OPEN both 1033** (first Mon–Thu overnight after Sep 13–15 recovery; Soft-530 CLEAR half-state **OVER** ~16:36):

| Surface | Living |
|---------|--------|
| **Soft-530** | **OPEN 530 / CF 1033** — `api.` / `app.` / `ops.` **`soft530_open_since` ~16:36** (Lookout + Lead; **was CLEAR**). Same-day CLEAR dwell **~13:21–16:36**. Do **not** stamp CLEAR at **~17:20** / **~17:40**. Morning ages from last relapse, **not** Sep 13. |
...
| **Lookout** | api `ok=false` **expected**; vault `ok=false` **expected** (dual OPEN both 1033). Brochure redirect `*/5` watch **GONE** — interim = plan-improve `*/20`. |
