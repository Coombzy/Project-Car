...
| P4-5 | **Synthetic monitors** | done | **Partial / paused.** Lookout **`projectcar-api-health-watch`** is **paused** (`enabled:false`). **Lead owns the interim morning/public probe** — that is the live path until the watch is re-armed. Lookout remains the owner when re-armed. Do **not** re-nag Ben to allow GET. `https://api.projectcar.ca/health` is **200 when Doc origin is up** (lid-close 502/530). Brochure homepage 200 optional. **Apex dropped** (sidecar deferred; do not monitor `/api/apex/health`). Discord invite check stays optional/deferred. Do not invent extra shipped monitors. Matches STATUS Live Shop API / Ops probe lines. |
...
| Date | Change |
|------|--------|
| 2026-09-08 | P4-5 Lookout watch **paused** (`projectcar-api-health-watch` `enabled:false`). Lead interim morning/public probe is the live path until re-armed. Do **not** re-nag Ben to allow GET. Lid-close restore: `doc-lid-restore.md`. Unfreeze: `doc-unfreeze.md`. Green shop-os-ci ≠ unfreeze. |
