...
| 10 | Basic uptime / synthetic monitoring | **Done / partial** — Lookout owns **`projectcar-api-health-watch`** (live flips on `https://api.projectcar.ca/health`; 200 when Doc origin is up; lid-close 502/530). Brochure Option A watch **to arm** while Soft-530 OPEN (only public LIVE surface; no continuous watch — only `*/20` smokes): [brochure-redirect-watch.md](brochure-redirect-watch.md) (flip Chief+Lead only; **not** companion re-ask). Homepage-only 200 is insufficient. Apex dropped (deferred). Discord invite check optional/deferred. Do not invent extra shipped monitors beyond that paper. |
