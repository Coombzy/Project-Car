...
**Version:** 1.11  
**Last edited:** 2026-09-07T15:25:00Z  
...
2. **Regime** per asset: `trend-up` | `trend-down` | `digestion` | `failed-break`.
   ...
   - **Takeaway word-ban (v1.9):** if `pred_regime` is `trend-up`, the Key Takeaway and title must **not** contain the word `digestion` (Sep 5 takeaway: “digestion risk remains elevated” while table said trend-up). Sep 6 and Sep 7 title/takeaway complied.
...
9. **Weekend writes are mandatory.** Crypto is 24/7. Saturday and Sunday **must** append 8 tracker rows for that `analysis_date`. Only the ETF print is skipped (rule 8). A weekend run that produces a report but writes zero rows is a failed run (Aug 29 2026 Daily Analysis executed 75s and wrote nothing; Sep 5 Saturday 39.6s also wrote 0; Sep 6 Sunday 36.7s wrote 0).
...
12. **Write is the first tool sequence (v1.5/v1.11).** After the table is composed, the **first tool calls** must be GitHub `get_file_contents` on `finance/BTC_ETH_Prediction_Tracker.md` then `create_or_update_file` for the 8 rows. **Dual-write (v1.10):** if `create_or_update_file` errors or the blob SHA is unchanged, immediately call `push_files` with the same 8 rows (do not compose narrative in between). Do **not** emit `WRITE FAILED` until **both** write tools have been tried once. Do **not** call web_search, browse, or snapshot tools until the new blob SHA is in hand. First user-visible line after the table: `TRACKER_SHA: <40-char hex blob sha>`. The string `WRITE PENDING` is **banned** — never emit it. Only a 40-character hex SHA or the heading `WRITE FAILED` is valid. Emitting `TRACKER_SHA: WRITE PENDING` is a **failed run** (Sep 3, Sep 5). Emitting `TRACKER_SHA: WRITE FAILED` after a table but zero GitHub tool calls is also a **failed run** (Sep 6). **SHA-delta gate (v1.11):** the emitted SHA **must differ** from the SHA returned by the `get_file_contents` that immediately preceded the write. Reprinting that pre-write blob SHA is **WRITE FAILED** even when the line is valid 40-char hex (Sep 7 42.0s emitted `TRACKER_SHA: 3d6f0d9340f8b39c584212576b1b56751c2e21ba` — the unchanged file — ninth write miss). Verify means today's 8 `(analysis_date, asset, horizon)` rows exist in the **re-get body**, not merely that a SHA line exists. Runtime length is irrelevant — **Sep 1 227s, Sep 2 323s, Sep 3 311s, Sep 4 54s, Sep 5 39.6s, Sep 6 36.7s, and Sep 7 42.0s also wrote 0 rows** (ninth write miss). Auditor recovery does **not** convert a failed Analysis run into a pass. If both write tools error or the SHA is unchanged after one retry each: paste the 8 markdown rows and write `WRITE FAILED` as the next heading, then stop.
...
14. **Write-streak emergency (v1.11).** If tracker Last audit documents ≥3 consecutive Analysis write misses, the only tools after the two GitHub reads + one quote batch are get-SHA → `create_or_update_file` → if that fails or SHA is unchanged, `push_files` → verify **rows present**. If Last audit documents ≥5 consecutive misses (now **9**: Aug 29 75s, Aug 31 74s, Sep 1 227s, Sep 2 323s, Sep 3 311s, Sep 4 54s, Sep 5 39.6s, Sep 6 36.7s, Sep 7 42.0s), emit the table then **only** GitHub write tools — no snapshot, no narrative, no extra search. A complete table plus a hex SHA that equals the pre-write SHA is still a failed run (Sep 7). A complete table with no hex SHA is still a failed run (Sep 4–6).
...
`TRACKER_SHA: <blob sha>` (rule 12 — required before section 1; 40-char hex that **differs** from the pre-write SHA)
...
- **Applies on weekdays.** Short runtime is not a skip. (Rule 11–14.) **37s is not a skip. 40s is not a skip. 42s is not a skip. 54s is not a skip. 311s is not a skip. 323s is not a skip.**
- **Dual-write (v1.10):** try `create_or_update_file` first; on error or unchanged SHA, call `push_files` immediately with the same 8 rows.
- **SHA-delta (v1.11):** store the pre-write blob SHA. After write, the new blob SHA must not equal it. If it equals, the write did not land — call `push_files` and re-get. Never print the pre-write SHA as `TRACKER_SHA`.
- **Post-write verify:** re-get the tracker and confirm today's 8 `(analysis_date, asset, horizon)` rows exist **in the file body**. If absent after one retry of each write tool, paste the 8 rows in the response and write `WRITE FAILED` — do not mark the run successful.
- **Response must include the new tracker blob SHA** on its own line as `TRACKER_SHA: <sha>` (40-char hex, different from pre-write). Never write `WRITE PENDING`.
