...
| **ListMachines** | ListMachines **EMPTY** / Docs **ABSENT** (Lead-verified **~16:51** after flap-while-connected; lid-restore **parked**). **CLEAR ≠ auto Doc wake.** Lightning **ABSENT**. `Mac.lan` may CONNECTED. `Mac.lan` ≠ `Docs-MacBook-Pro` ≠ `lightning`. Ben asked once Doc awake/desktop — **no re-nag**. **Not** camp GO. |
