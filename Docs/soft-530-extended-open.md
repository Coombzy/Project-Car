...
**Updated:** 2026-09-15  
**Related:** `STATUS.md` (living dual-OPEN + this pointer), [soft-530-weekday-overnight.md](soft-530-weekday-overnight.md) (**Mon–Thu nights** after last weekday `*/20` ~17:40 MT until next weekday first fire ~06:00 — Lookout api+vault only; Friday last `*/20` → weekend coverage; this lock is **not** that coverage card), [doc-reappear-first-hop.md](doc-reappear-first-hop.md) (**first hop** when `Docs-MacBook-Pro` reappears — machineId `95a229f5-9296-4a18-aa98-70fd300dabdf`; **never** `Mac.lan` / Porsche), [mcking-reappear-first-hop.md](mcking-reappear-first-hop.md) (**first hop** when `lightning` / McKing reappears — machineId `9067d14b-46e5-4ef0-82d5-fce0febdc8f7`; **never** `Mac.lan`), [soft-530-clear-smoke.md](soft-530-clear-smoke.md) (**first** recovery smoke after extended-OPEN Soft-530 CLEAR — **before** desk / **#79.1** / unfreeze), [vault-clear-smoke.md](vault-clear-smoke.md) (**first** recovery smoke after extended-OPEN vault CLEAR — **before** Bitwarden / desk), [dual-host-outage.md](dual-host-outage.md) (wake order — **not** this lock), [vault-stay-up.md](vault-stay-up.md) (McKing half + forensics `@d88cacb`), [brochure-redirect-watch.md](brochure-redirect-watch.md) (Option A Lookout **spec** — living watch **GONE**), [lookout-rearm-sop.md](lookout-rearm-sop.md) (never delete to fix cron; **HOLD** after one Ben ask; weekday Chief `*/20`; overnight/weekend Option A **UNCOVERED**; **#78 LIVE-SUPERSEDED**; **re-arm success ≠ first fire** — stickiness ≥3 consecutive `*/5` windows; chronic stall ≤1 pause/resume per calendar-day, clock resets **America/Edmonton midnight**; Lead may GO in-place re-arm of **both** api + vault when stalled — **not** after today's 1× budget), [overnight-baseline-stamp.md](overnight-baseline-stamp.md) (Lookout field may be **armed** **or** **chronic HOLD** — `enabled:true` + cron-dead after the day's 1× re-arm; chronic HOLD ≠ Ben ping; **home-off Soft-530 + Docs ABSENT = SAME-CLASS expected**), [home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md) (**home-off:** Docs **ABSENT** + Soft-530 **OPEN** is **EXPECTED**; quiet-ops **HOLD**; do **not** re-nag lid-restore / KeepAlive; complement to `fb86f90`, **not** a re-cutover), [post-dual-clear-go.md](post-dual-clear-go.md) (**CLEAR path** — **never auto-fire**), [doc-lid-restore.md](doc-lid-restore.md) (Lead process wake on the **asserted** host — **not** queued from home-off ABSENT duration), [deployment-guide.md](deployment-guide.md) (Monday paper map)
...
**Home-off align** ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)): Docs **ABSENT** + Soft-530 **OPEN** at home is **EXPECTED**. Quiet-ops **HOLD** for that class even when `lightning` is also on ListMachines and even if vault later CLEARs independently on McKing. Do **not** re-nag lid-restore / KeepAlive / Doc-wake because home-off lasted another day. Do **not** auto-queue [doc-reappear-first-hop.md](doc-reappear-first-hop.md) from ABSENT duration — that hop waits on actual reappear. Lookout Soft-530 `ok=false` while Docs **ABSENT** is **expected baseline**, not stickiness panic — do **not** re-arm Soft-530 companions.
...
| # | Lock | Meaning |
|---|------|---------|
| **1** | **No Ben re-nag** | Doc-wake + McKing wake **already asked**. Do **not** page Ben again because the clock crossed ~24h. **Home-off** Soft-530 **OPEN** + Docs **ABSENT** is **EXPECTED** — do **not** re-nag lid-restore / KeepAlive ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)). |
| **2** | **Lead + Chief stay armed** | **Lead** stays armed for ListMachines **`Docs-MacBook-Pro`** reappear → [doc-reappear-first-hop.md](doc-reappear-first-hop.md) (assert machineId **`95a229f5-9296-4a18-aa98-70fd300dabdf`**, **never** `Mac.lan` / Porsche) **then** [doc-lid-restore.md](doc-lid-restore.md) **process wake only**. Armed **for actual reappear**, **not** for home-off ABSENT duration ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)). **Chief** stays armed for **`lightning`** reappear → [mcking-reappear-first-hop.md](mcking-reappear-first-hop.md) (assert machineId **`9067d14b-46e5-4ef0-82d5-fce0febdc8f7`**, **never** `Mac.lan`) **then** [vault-stay-up.md](vault-stay-up.md) vault wake — vault may CLEAR or OPEN **independently** of Soft-530 at home. Identity lock unchanged — no `Mac.lan` Shell. |
| **3** | **Lookout continues; companions HOLD until Ben reopens** | Lookout `api.` `/health` + vault `/alive` stay **armed** — **never delete** them to fix stalled cron ([lookout-rearm-sop.md](lookout-rearm-sop.md)). **`enabled:true` ≠ firing.** Quiet-ops **continues** = stay armed **and** stay firing with **stickiness** (re-arm success ≠ first fire: `lastRun` advances across **≥3 consecutive** `*/5` windows, ≥10–15m). **Chronic-HOLD exception:** when **both** watches are calendar-day chronic HOLD (`enabled:true` but cron-dead after today's **1×** re-arm budget), “Lookout continues” is **false for flip detection** — see the exception table below. Living 2026-09-15: both api + vault are **chronic HOLD today**. Living ~09:14 MT: vault re-arm ~08:50 → first fire ~08:55 → stall ~22m; api stalled ~56m since ~08:21 — same class; day's 1× budget spent. Lead **may GO** in-place re-arm of **both** watches when stalled **only if** the calendar-day 1× is unused, then verify stickiness before healthy. Chronic stall (again within 24h) = **≤1** pause/resume per calendar-day (clock resets **America/Edmonton midnight**); Lead paper only; interim Chief `*/20` + Lead probes; **HOLD** Ben recreate. Brochure-redirect **paper** **continues** ([brochure-redirect-watch.md](brochure-redirect-watch.md)). Living Option A continuous watch is **GONE** (deleted to fix stalled cron; Auto-review blocked recreate) — **HOLD** after one Ben ask; weekday interim = Chief `*/20` redirect smoke. Overnight + weekend Option A Dynamic redirect is **UNCOVERED — accept that risk** ([soft-530-weekday-overnight.md](soft-530-weekday-overnight.md) · `84107e7`). Do **not** invent overnight plan-improve, Zone, or Garage coverage. **Next Option A smoke** = first weekday `*/20` after ~06:00, **or** Ben-approved Lookout recreate. Soft-530 companions stay **HOLD / not armed** until **Ben reopens** — **not** weekend-only. Do **not** re-ask. **#78 lookout-resume** is **LIVE-SUPERSEDED** (api-health already `enabled:true` — can still cron-stall). |
...
## Chronic-HOLD exception — “Lookout continues” is false for flip detection

Living quiet-ops (`1e6a0b1`) says Soft-530 extended OPEN → Lookout **continues**. **Exception:** when **both** `projectcar-api-health-watch` **and** `projectcar-vault-health-watch` are **calendar-day chronic HOLD** (`enabled:true` but cron-dead after today’s **1×** re-arm budget), “Lookout continues” is **false for flip detection**. Quiet-ops itself still holds (no Ben re-nag; duration ≠ GO; companions HOLD). **Chronic Lookout ≠ quiet-ops failure ≠ Ben ping.**

This fold is **paper only**. It does **not** pause, resume, recreate, or mutate Lookout. It does **not** execute Zone or Garage. It does **not** re-ask Ben.

| # | Exception | Meaning |
|---|-----------|---------|
| **1** | Interim Soft-530 + vault SSOT smoke | Chief weekday `*/20` **+** Lead coding-progress probes. Accept **≤~20m** flip latency. Not a Lookout `*/5`. Cross-link [lookout-rearm-sop.md](lookout-rearm-sop.md) (until stickiness) · [overnight-baseline-stamp.md](overnight-baseline-stamp.md) (Lookout field). |
| **2** | Chronic HOLD ≠ Ben recreate | Do **not** reopen a Ben recreate ask. Quiet-ops + Auto-review lesson stand. Brochure recreate already one-ask **HOLD**. |
| **3** | Calendar-day clock | **1×** pause/resume per calendar-day resets **America/Edmonton midnight**. Next local day Lead **may** try **one** in-place re-arm again, then stickiness **≥3** consecutive `*/5` windows before healthy ([lookout-rearm-sop.md](lookout-rearm-sop.md) row 8). Today’s budget spent → Lead **paper only** until midnight. |
| **4** | Companions + brochure recreate stay HOLD | Soft-530 ops/app companions **HOLD / not armed**. Brochure Option A recreate **HOLD**. Do **not** re-ask. Do **not** invent companion GO from chronic Lookout. |

Living 2026-09-15 honesty: Soft-530 **OPEN** 530/1033; vault **OPEN** 502; ListMachines **Mac.lan + lightning** / Docs **ABSENT**; Option A Redirect **PASS**; Soft-530 UX **LIVE**; Lookout **both chronic HOLD today**; **#82** Ben GO only.

Stamp the Lookout field as **chronic HOLD** (not “armed and firing”) when this exception is live — [overnight-baseline-stamp.md](overnight-baseline-stamp.md). Same-class chronic HOLD on morning `*/20` = **stay quiet**. Do **not** Ben-nag from that field.
...
| **Home-off Soft-530 as recovery queue** | Docs **ABSENT** + Soft-530 **OPEN** at home is **EXPECTED** ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md)). Do **not** re-nag lid-restore / KeepAlive. Do **not** treat home 1033 as camp cutover failure. Do **not** re-arm Soft-530 companions from expected `ok=false`. |
...
| **Drop Lookout** | `api.` + vault stay armed. Brochure-redirect **paper** continues. Continuous Option A watch is **GONE** — weekday Chief `*/20`; overnight/weekend **UNCOVERED** ([lookout-rearm-sop.md](lookout-rearm-sop.md)). |
| **Treat chronic HOLD as quiet-ops failure** | Chronic Lookout ≠ quiet-ops failure ≠ Ben ping. Interim = Chief `*/20` + Lead probes (accept ≤~20m). |
...
| **Call healthy on first fire** | Re-arm success = stickiness ([lookout-rearm-sop.md](lookout-rearm-sop.md) row 8). Living vault: ~08:55 first fire then ~22m stall. |
| **Loop pause/resume on chronic** | Stall again within 24h → **≤1** pause/resume per calendar-day. Clock resets **America/Edmonton midnight**. Lead paper only today after 1×. **HOLD** Ben recreate. |
| **Reopen Ben recreate from chronic HOLD** | Chronic HOLD does **not** reopen a Ben recreate ask. Companions + brochure recreate stay **HOLD**. |
...
- Re-nag lid-restore / KeepAlive / Doc-wake because home-off Soft-530 lasted another day ([home-vs-camp-doc-posture.md](home-vs-camp-doc-posture.md))
- Auto-queue [doc-reappear-first-hop.md](doc-reappear-first-hop.md) from Docs **ABSENT** duration — hop waits on actual reappear
- Treat Lookout Soft-530 `ok=false` while Docs **ABSENT** at home as stickiness panic, or re-arm Soft-530 companions
- Treat home Soft-530 **OPEN** as camp cutover failure
...
- Claim “Lookout continues” for flip detection while **both** watches are chronic HOLD
...
- Call a re-armed api or vault watch healthy on first fire (require stickiness — ≥3 consecutive `*/5` windows)
- Loop pause/resume more than **1×/calendar-day** on a chronic stall (clock resets America/Edmonton midnight)
- Reopen a Ben recreate ask because api + vault are chronic HOLD today
