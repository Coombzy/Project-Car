...
**Last audited:** 2026-09-21 (Chief probe ~06:46 America/Edmonton — Brochure Option A **LIVE** at edge. Zone Redirect **301** pack apex+www owns pretty URLs (`/` → `/index.html`, `/shop` → `/the-shop.html`, `/membership` → `/membership.html`, `/about` → `/about.html`). Live assets: `styles.css?v=36` + `waitlist.js?v=3` Soft-530 Discord+mailto UX. Home still **STALE** until **#82** Worker-live (bare `href="/"` + apex canonical/og still `https://projectcar.ca/`). Do **not** claim live Worker `_redirects` 302 `/shop` or `styles.css?v=35`. SSOT: `brochure-option-a-live.md`. Soft-530 **OPEN** `home_expected` when Doc origin is down — quiet-ops, not an incident. P4-2 Discord equal CTA still live. P2-4 headers still. P3-4 Shop facility bullets still. P3-2 / P3-3 About stay **held / deferred**. Open **P2-8** Option A HTML finish.)  
...
- `brochure-option-a-live.md` — Option A Zone 301 **LIVE** at edge (pretty-URL SSOT; Worker `_redirects` stay thin; Home leftovers = **#82**)
...
| Check | Result |
|--------|--------|
| https://projectcar.ca / www | Option A **LIVE** at edge (Zone Redirect **301** pack apex+www: `/` `/shop` `/membership` `/about` → `*.html`). Worker `projectcar-brochure` still serves HTML. Live assets: `styles.css?v=36` + `waitlist.js?v=3`. Do **not** claim live Worker `_redirects` 302 `/shop` or `styles.css?v=35`. Home leftovers (**#82**): bare `href="/"` + canonical/og `https://projectcar.ca/`. Earlier ships still on Worker: P4-2 Discord CTA (#67); P3-4 (#65); P3-5 (#62); P3-1 (#61); P2-4; P2-3 (#57); P2-2 (#56); P2-1 (#55); P1-1 / P1-3 (#52). HTML from this agent often hits CF challenge (403); SSL in front is Cloudflare. SSOT: `brochure-option-a-live.md`. |
...
| P2-8 | **Option A HTML finish** | blocked | Home nav / canonical / og / sitemap still use bare `/` and `https://projectcar.ca/` (not `index.html`). Zone 301 already makes `/` work. Finish is **#82** merge + Zone Worker Direct Upload of that tree. Do **not** implement HTML from a docs PR. Do **not** treat merge of **#82** as Worker-live. Record: `brochure-option-a-live.md`. |
...
| P4-2 | **Discord as equal CTA on Home/Membership** | done | Discord equal CTA is still **LIVE** (Garage PR #67 → `2b772ff`: Home `cta-hero` Join the waitlist + Join Discord; Membership Discord contact-primary + alongside-waitlist). Pretty URLs are no longer Worker `_redirects` 302 `/shop` — Zone Option A **301** pack is the live SSOT (`brochure-option-a-live.md`). Live cache is `styles.css?v=36` + `waitlist.js?v=3` (Website **#80**), not `styles.css?v=35`. Home leftovers (nav/canonical/og/sitemap → `index.html`) are **P2-8** / **#82**. P3-2 / P3-3 About stay held/deferred. P1-6 stays deferred. |
...
1. **P2-8** Option A HTML finish — **blocked** on **#82** merge + Zone Worker Direct Upload (do not implement HTML from a docs PR)  
2. P3-2 / P3-3 About — **held / deferred**  
3. P4 remaining as interest grows (P4-2 **done**)  
4. P1-6 only **after Member cutover GO** — still deferred; no brochure HTML from a docs PR.

Brochure hygiene is **already live** on Worker `projectcar-brochure` (re-uploaded 2026-09-06). P1-1 / P1-3 are **LIVE** on that Worker (PR #52 / `5361212`; 2026-09-07 smoke PASS). P2-1 is **LIVE** on that Worker (PR #55 / `453e44d`; Zone smoke PASS). P2-2 is **LIVE** on that Worker (PR #56 / `ed41d23`; Zone Direct Upload ~2026-09-07 15:48 America/Regina; Zone smoke PASS). P2-3 is **LIVE** on that Worker (PR **#57** / `be60a01`; Zone Direct Upload ~2026-09-07 16:01 America/Regina; smoke PASS: `styles.css?v=34` + `banner-logo.png?v=30` on public HTML — Home/About/Contact at minimum; apex+www). P2-4 is **LIVE** on that Worker (2026-09-07 smoke PASS — Transform Rules `brochure-security-headers` / `brochure-html-no-cache` / `brochure-asset-immutable`; apex+www; Worker **200**s). P3-1 is **LIVE** on that Worker (PR **#61** / `88c9820`; Zone Direct Upload ~16:50 America/Regina; smoke PASS: Home pitch + `cta-hero` Join the waitlist + Project Underway + Shop/Membership pointers; no feature-bullet dump). P3-5 is **LIVE** on that Worker (PR **#62** / `9d83bfc`; Zone Direct Upload; smoke PASS: Shop/Membership/Roadmap use “24/7 shop access (when operational)”; Home (and About/Contact) have no absolute 24/7). P3-4 is **LIVE** on that Worker (PR **#65** / `f191ad5`; Zone Direct Upload ~18:46 America/Regina; smoke PASS apex+www: Shop 200 facility bullets (bays/hoists, shared fab, safety/cleanliness, structured shop) + honesty “intended shop capability — not live yet”; no 24/7 packaging on Shop; Membership still “24/7 shop access (when operational)”; Home still P3-1 pitch + `cta-hero` waitlist + Project Underway; no capability-bullet dump; `styles.css?v=34`; P2-4 headers still present). P4-2 Discord equal CTA is **LIVE** on that Worker (PR **#67** / `2b772ff`). Current pretty-URL SSOT is Zone Option A **301**s (`brochure-option-a-live.md`) — do **not** treat Worker `_redirects` 302 `/shop` or `styles.css?v=35` as current live. Live cache is `styles.css?v=36` + `waitlist.js?v=3`. **P2-8** Option A HTML finish is **open / blocked** on **#82**. Ops + Member booking are live on Doc demo **and** the temporary `app.` alias. Intended management host is **`ops.`**. Mission Control cockpit still needs **Ben GO**. The shop is not open.

Waitlist (P1-5), Chat strip (P1-2), Home CTA weight (P1-1), Contact channel honesty (P1-3), absolute OG (P2-1), canonicals (P2-2), cache `?v=` SSOT (P2-3), security + cache-split headers (P2-4), Home pitch + CTA (P3-1), 24/7 tense (P3-5), Shop vs Home split (P3-4), Discord equal CTA (P4-2), git home (P2-7), and P0-3…P0-7 are **done** (git + live Worker). **P2-8** Option A HTML finish is **open / blocked** on **#82**. P3-2 / P3-3 About stay **held / deferred**. P1-6 stays **deferred**. Apex (P0-1 / P0-2) is **deferred** — do not start the queue there.  
...
curl -sSI "https://projectcar.ca/styles.css?v=36"
...
| Date | Change |
|------|--------|
| 2026-09-21 | Live-audit Option A **LIVE** at edge (Chief probe ~06:46 America/Edmonton). Zone 301 pretty-URL pack is SSOT (`/` `/shop` `/membership` `/about` → `*.html`). `styles.css?v=36` + `waitlist.js?v=3` Soft-530 Discord UX live. Snapshot/P4-2 no longer claim Worker `_redirects` 302 `/shop` or `styles.css?v=35` as current. Open **P2-8** Option A HTML finish (blocked on **#82** merge + Worker Direct Upload). SSOT: `brochure-option-a-live.md`. |
