...
| [doc-lid-restore.md](doc-lid-restore.md) | Ordered Lead wake/restore after lid-close / morning **530 / 1033**. Sequence only — process wake, **not** a `git pull`. Weekend `/health` flip (plan-improve off Sat/Sun) uses this same wake — **not** unfreeze, **not** **#82**. Essays stay in api-stay-up / shop-web-stay-up / doc-software-baseline. After Ben GO, pull is [doc-unfreeze.md](doc-unfreeze.md). |
...
| [api-stay-up.md](api-stay-up.md) | Keep https://api.projectcar.ca up (uvicorn on Doc; Zone owns tunnel). Weekend flip coverage = Lookout `api.` `/health` + vault `/alive` **only**. Brochure is Worker `projectcar-brochure`, not Doc `:8088`. |
