...
| **Chronic stall** | Stall again within **24h** of a re-arm that reached first fire or stickiness. **≤1** pause/resume per calendar-day. Lead **paper only**. | Looping pause/resume. Ben recreate. Zone / Garage / docker / Doc-wake. |
...
**Chronic stall class:** if a watch stalls again **within 24h** of a re-arm that reached first fire **or** stickiness, do **not** loop pause/resume more than **1×/calendar-day**. Classify **chronic**. Lead **paper only**. Interim = Chief `*/20` + Lead coding-progress probes. **HOLD** Ben recreate. Do **not** invent Zone / Garage / docker / Doc-wake from a chronic stall.

**Lead may GO** in-place re-arm of both `projectcar-vault-health-watch` **and** `projectcar-api-health-watch` when stalled, then **verify stickiness** before calling healthy. This paper does **not** execute that GO.
...
| **8** | **Re-arm success ≠ first fire; require stickiness. Chronic ≤1 pause/resume per calendar-day** | After in-place re-arm, success = `lastRun` advances across **≥3 consecutive** schedule windows (`*/5`: ≥3 fires spanning **≥10–15m**). Living ~09:14 MT 2026-09-15: vault re-arm ~08:50 → first fire ~08:55 OK → stall again (~**22m**). If a watch stalls again **within 24h** of a re-arm that reached first fire or stickiness: classify **chronic**; do **not** loop pause/resume more than **1×/calendar-day**; Lead **paper only**; interim = Chief `*/20` + Lead coding-progress probes; **HOLD** Ben recreate. Do **not** call healthy on first fire. |
...
|---------|------------------------|
| Soft-530 `api.` `/health` | **Stay armed.** Never delete. **`enabled:true` ≠ firing.** Living ~09:14: stalled ~**56m** since ~08:21 — same class as vault. Lead **may GO** in-place re-arm; stickiness before healthy. Remaining **OK-ish** is **not** living contrast. |
| Vault `/alive` | **Stay armed.** Never delete. **`enabled:true` + cron-stalled ≠ HTTP 502.** First fire ≠ healthy. Lead **may GO** in-place re-arm (pause/resume / re-save); verify stickiness (row 8). Chronic ≤1 pause/resume per calendar-day. Interim = Chief `*/20` + Lead coding-progress probes — **not** a Ben ping. |
...
| **Chronic stall ≤1 pause/resume per calendar-day** | Stall again within **24h** of a re-arm that reached first fire or stickiness → classify **chronic**; Lead **paper only**; interim Chief `*/20` + Lead probes; **HOLD** Ben recreate. Do **not** loop pause/resume. |
...
| **Not loop pause/resume on chronic** | Stall again within 24h → **≤1** pause/resume per calendar-day. Lead paper only. |
...
- Loop pause/resume more than **1×/calendar-day** on a chronic stall (stall again within 24h)
