...
## Living now (~07:25 America/Edmonton) — blip recover

Soft-530 **CLEAR** **`soft530_clear_since` ~07:25** (Lead `api.` `/health` **200** ok; `app.` + `ops.` **307**; dwell **≥51m** at **~08:16**). **`soft530_open_blip` ~07:06–~07:25**. Prior overnight CLEAR dwell **~22:00–07:06** (~9h) **ENDED** at the blip. Vault still **OPEN 530 / CF 1033** — **EXPECTED**. ListMachines **EMPTY**. **EMPTY ≠ Soft-530 OPEN.** **CONNECTED ≠ Shellable ≠ tunnel healthy.** CLEAR+EMPTY = **half-ops HOLD**. Camp / [post-dual-clear-go.md](post-dual-clear-go.md) **must not reopen**. Re-offer camp vault-on-Doc Ben GO only after Docs **CONNECTED+Shellable** + clear-smoke + ≥30m SAME-CLASS dwell (dwell **PASS**; Shellable **fail**). Shared OPEN-since-**~07:06** queue **cancelled**. Option A Redirect+UX **PASS**. Home **STALE** until **#82**. **`freeze_build_id_public=true`** `` `5swmVz-T2CqKEQzTk1ifU` `` (public MATCH). **`freeze_git_verified`** queued (behind intact). Public MATCH ≠ live-verify PASS / ≠ Shared amend. **`shared_fleet_card_stale=true`**. Half-state ≠ dual CLEAR. ≠ auto camp / **#82** / **#79.1** / unfreeze / Bitwarden. Essay: [soft530-clear-vs-local-exec.md](soft530-clear-vs-local-exec.md) · [freeze-build-id-public-probe.md](freeze-build-id-public-probe.md).
